...
function getWsgiUrl(location: Location, name) {
  ...
  return `${location.protocol}//${location.hostname}:${location.port}${pathname}plugins/react_wsgi_dashboards/${name}/index.html`;
}
...
buildbotSetupPlugin((reg, config) => {
  const wsgi_dashboards = config.plugins['react_wsgi_dashboards'];
