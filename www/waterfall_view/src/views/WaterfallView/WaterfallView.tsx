...
import {FaChartBar, FaSearchMinus, FaSearchPlus} from 'react-icons/fa';
...
const isBuilderFilteredHasMaster = (
  builder: Builder,
  masters: DataCollection<Master>,
  showOldBuilders: boolean,
) => {
...
export const WaterfallView = observer(() => {
  ...
  const [scalingFactor, setScalingFactor] = useState(() =>
    settings.getFloatSetting('Waterfall.scaling_waterfall'),
  );
  ...
  const queriesResolved = buildersQuery.resolved && mastersQuery.resolved && buildsQuery.resolved;
    ...
    return autorun(() => {
      ...
      const buildersToShow = buildersQuery.array.filter((b) =>
        isBuilderFiltered(
          ...
          mastersQuery,
          builderToBuilds,
          showBuildersWithoutBuilds,
          showOldBuilders,
        ),
      );
      const builderIds = new Set(buildersToShow.map((b) => b.builderid));
      const buildGroups = groupBuildsByTime(
        builderIds,
        buildsQuery.array,
        settings.getIntegerSetting('Waterfall.idle_threshold_waterfall'),
        Date.now() / 1000,
      );
  ...
  useEffect(() => {
    visualizer.onViewConfigMaybeUpdate(
      rootRef.current,
      headerRef.current,
  ...
  return (
    <div ref={rootRef} className="container bb-waterfall-view">
      <div>
        <p>Tags: {filterManager.getElementsForTags(getAllTags(buildersForTags))}</p>
      </div>
