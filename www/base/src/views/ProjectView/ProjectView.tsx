...
import {ProjectPendingBuildRequestsWidget} from '../../components/ProjectWidgets/ProjectPendingBuildRequestsWidget';
import {ProjectDescriptionWidget} from '../../components/ProjectWidgets/ProjectDescriptionWidget';

const defaultWidgetsConfig = ['description', 'builders', 'pending_build_requests', 'changes'];
...
export const ProjectView = observer(() => {
  ...
  useTopbarItems([
    ...
    {
      caption: project !== null ? project.name : '...',
    ...
    return (
      <div className="bb-project-view-container">
        <Alert variant="danger">
          ...
          <p>
            Check <code>c["www"]["project_widgets"]</code> field in Buildbot configuration
          </p>
        </Alert>
      </div>
    );
  ...
  const renderWidgetNameConfig = (name: string) => {
    ...
    return <Alert variant="danger">Unknown widget {name}</Alert>;
  };
