    ...
    return masterIdToConnectedWorkers[master.id].map((worker) => (
      <span key={worker.id}>
        <Link to={`/workers/${worker.id}`}>
          <BadgeRound className="results_SUCCESS">{worker.name}</BadgeRound>
        </Link>
      </span>
    ));
  };
  ...
  const renderMaster = (master: Master) => {
    return (
      <tr key={master.id}>
        ...
        <td>{workersQuery.resolved ? renderWorkersForMaster(master) : <LoadingSpan />}</td>
        <td>
          {master.last_active !== null ? durationFromNowFormat(master.last_active, now) : 'Unknown'}
        </td>
      </tr>
    );
  };
