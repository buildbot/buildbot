...
const getBuildTimeElement = (build: Build, buildTimeBase: string, now: number) => {
  if (buildTimeBase === BUILD_TIME_BASE_COMPLETE_TIME) {
    return build.complete ? (
      <span title={dateFormat(build.complete_at!)}>{dateFormat(build.complete_at!)}</span>
    ) : (
      <span title={dateFormat(build.started_at)}>Started at {dateFormat(build.started_at)}</span>
    );
  }
  return (
    <span title={dateFormat(build.started_at)}>{durationFromNowFormat(build.started_at, now)}</span>
  );
};

export const BuildsTable = observer(
  ({builds, builders, fetchLimit, onLoadMore}: BuildsTableProps) => {
    const now = useCurrentTime();
    const sortedBuilds = builds.array.slice().sort((a, b) => b.started_at - a.started_at);

    const buildTimeBase = buildbotGetSettings().getChoiceComboSetting(
      'BuildsTable.build_time_base',
    );

    const rowElements = sortedBuilds.map((build) => {
      const builder = builders === null ? null : builders.getByIdOrNull(build.builderid.toString());
      const builderNameElement =
        builders === null ? <></> : <td>{builder === null ? '' : builder.name}</td>;
      ...
      return (
        <tr key={build.id}>
          ...
          <td>{getBuildTimeElement(build, buildTimeBase, now)}</td>
          <td>{buildCompleteInfoElement}</td>
          <td>{getPropertyValueOrDefault(build.properties, 'revision', '(unknown)')}</td>
          <td>
            {getPropertyValueArrayOrEmpty(build.properties, 'owners').map((owner, index) => (
        ...
        <Table hover striped size="sm">
          <tbody>
            <tr>
              ...
              <td width="100px">#</td>
              <td width="150px">
                {buildTimeBase === BUILD_TIME_BASE_COMPLETE_TIME ? 'Completed At' : 'Started At'}
              </td>
