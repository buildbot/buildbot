...
export const MultipleWorkersActionsModal = observer(
  ({workers, preselectedWorkers, onClose}: MultipleWorkersActionsModalProps) => {
    if (workers.length === 1) {
    ...
    const stopDisabled =
      selectedWorkers.length <= 0 || selectedWorkers.every((w) => w.connected_to.length === 0);
    ...
    const doAction = async (method: string) => {
      ...
      const results = await Promise.allSettled(
        selectedWorkers.map((w) => w.control(method, {reason: reasonText})),
      );
    ...
    return (
      <Modal size="lg" show={true} onHide={() => onClose()}>
        <Modal.Header closeButton>
          <Modal.Title>
            Worker actions for {workers.length === 1 ? workers[0].name : '...'}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <div className="form-horizontal">
            {errors === null ? (
              ...
              <div ng-show="error" className="alert alert-danger">
                {errors}
              </div>
            )}
            {workers.length !== 1 ? (
            ...
            <label htmlFor="reason" className="control-label col-sm-2">
              Reason
            </label>
