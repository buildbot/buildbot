...
export const getWorkerStatusIcon = (worker: Worker, onClick: () => void) => {
  if (worker.paused) {
    return (
      <FaPause title="paused" onClick={onClick}>
        &nbsp;
      </FaPause>
    );
  }
  if (worker.graceful) {
    return <FaStop title="graceful shutdown" onClick={onClick} />;
  }
  if (worker.connected_to.length > 0) {
    return <FaRegSmile onClick={onClick} />;
  }
  return <></>;
};
  ...
  ({
    ...
    const renderWorkerInfos = (worker: Worker) => {
      return workerInfoNamesToDisplay.map((name) => {
        ...
        return (
          <td key={'info-' + name}>
            {name === 'access_uri' ? <a href={info}>{info}</a> : <>{info}</>}
          </td>
        );
      });
    };
    ...
    const renderPauseReason = (worker: Worker, displayPauseReason: boolean) => {
      ...
      return <td key="worker-paused">{worker.paused ? worker.pause_reason : ''}</td>;
    };
    ...
    const renderWorkerRow = (worker: Worker, displayPauseReason: boolean) => {
      return (
        <tr key={worker.name}>
          ...
          <td key="name">
            <Link to={`/workers/${worker.workerid}`}>{worker.name}</Link>
          </td>
        ...
        <thead>
          <tr>
            ...
            {workerInfoNamesToDisplay.map((name) => (
              <th key={'info-' + name}>{name}</th>
            ))}
          </tr>
        </thead>
        <tbody>{workers.map((worker) => renderWorkerRow(worker, displayPauseReason))}</tbody>
      </Table>
    );
  },
);
