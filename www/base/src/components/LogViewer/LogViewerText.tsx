...
export const LogViewerText = observer(
  ({
    log,
    downloadInitiateOverscanRowCount,
    downloadOverscanRowCount,
    cachedDownloadOverscanRowCount,
    cacheRenderedOverscanRowCount,
    maxChunkLinesCount,
  }: LogViewerProps) => {
    ...
    if (managerRef.current === null) {
      managerRef.current = new LogTextManager(
        (offset, limit) =>
          accessor.getRaw(`logs/${log.logid}/contents`, {
            offset: offset,
            limit: limit,
          }),
        log.type,
    ...
    const LogTextArea: React.FC<Size> = ({height, width}) => (
      <div style={{width: width, height: height}} ref={containerRef}>
        <div className="bb-logviewer-text-download-log">
          <div>
            <LogSearchField
              currentResult={manager.currentSearchResultIndex + 1}
        ...
        >
          {({index, style}) =>
            LogViewerTextLineRenderer({
              manager: manager,
              logLineDigitCount: logLineDigitCount,
              style: style,
              index: index,
            })
          }
    ...
    return (
      <>
        ...
        <AutoSizer>{LogTextArea}</AutoSizer>
      </>
    );
  },
);
