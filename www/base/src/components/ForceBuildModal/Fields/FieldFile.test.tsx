...
import {FieldFile, MAX_DISPLAY_CHAR} from './FieldFile';
...
async function assertRenderToSnapshot(
  ...
  const field: ForceSchedulerFieldFile = {
    name: 'dummy',
    fullName: 'fullDummy',
    label: 'dummyLabel',
  ...
  const component = render(<FieldFile field={field} fieldsState={state} />);
  expect(component.asFragment()).toMatchSnapshot();

  if (updateValue !== undefined) {
    const expectedState = updateValue;
    const input = component.getByTestId(`force-field-${field.fullName}-text`);
...
describe('ForceFieldFile component', function () {
  ...
  it('does not display too large default value', async () => {
    const unsafeValue = 'test'.repeat(MAX_DISPLAY_CHAR);
    await assertRenderToSnapshot('default', unsafeValue);
  });
});
