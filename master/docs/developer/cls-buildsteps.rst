...
.. py:class:: BuildStep(name, description, descriptionDone, descriptionSuffix, locks, haltOnFailure, flunkOnWarnings, flunkOnFailure, warnOnWarnings, warnOnFailure, alwaysRun, progressMetrics, useProgress, doStepIf, hideStepIf)
    ...
    .. py:method:: setBuildWorker(build)

        :param build: the :class:`~buildbot.buildworker.BuildWorker` instance on which this step will run.

        Similarly, this method is called with the build worker that will run this step.
        The default implementation sets the :attr:`buildworker` attribute.

    .. py:attribute:: buildworker

        The build worker that will run this step.
    ...
    .. py:method:: startStep(remote)
        ...
        :param remote: a remote reference to the worker-side
            :class:`~buildworker.bot.WorkerBuilder` instance
        :returns: Deferred
    ...
    .. py:method:: workerVersionHasCommand(command)

        :param command: command to examine
        :type command: string

        This method raise BuildWorkerTooOldError if ``command`` is not implemented on the worker

    .. py:method:: getWorkerName()

        :returns: string

        Get the name of the buildworker assigned to this step.
    ...
    .. py:method:: runCommand(command)
        ...
        This method connects the given command to the step's buildworker and runs it, returning the Deferred from :meth:`~buildbot.process.remotecommand.RemoteCommand.run`.
