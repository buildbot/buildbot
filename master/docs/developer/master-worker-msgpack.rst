...
Worker sends this message when trying to login to master.

``op``
    Value is a string ``challenge``.
...
Master returns ``result`` value, which is randomly generated bytes string needed for worker-master authentication.
More on authentication in section `_MsgPack_Authentication_Message`.
...
The authentication message requests master to authenticate username and hash given by the worker.
More on authentication in section :ref:`MsgPack_Authentication_Message`.
...
``hash``
    Value is a string.
    It represents an updated worker password, hashed with a challenge which was given by the master previously in the ``challenge`` call.
    More on authentication in section :ref:`MsgPack_Authentication_Message`.
...
.. _MsgPack_Authentication_Message:

Authentication
--------------

    When worker tries to login to master, it sends the request message ``op``:”challenge”.
    Master responds by returning randomly generated data to worker and saves that data on its side as well.
    Worker updates its password with given data from master and hashes it with algorithm md5.
    Then worker sends the message with ``op``:”auth”, its username, seq_number of a message and a hash value which worker calculated.

    Master has a list of workers and their passwords which are allowed to login to that master.
    When master gets the message with ``op``:”auth”, it checks if there is such a name in the workers list.
    If worker is in the list, master takes its password, updates and hashes it with the data it calculated before on the message ``op``:”challenge”.
    Master compares its hash with the hash it got from worker in the ``auth`` message “hash” value.

    If the value from worker is the same as calculated in the master, master accepts the login of that worker and returns ``op``:“response” message with ``result``:``None``.
