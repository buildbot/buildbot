...
* The algorithm to match build requests to workers has been rewritten :pull:`615`.
  The new algorithm automatically takes locks into account, and will not schedule a build only to have it wait on a lock.
  The algorithm also introduces a ``canStartBuild`` builder configuration option which can be used to prevent a build request being assigned to a worker.
...
* :bb:sched:`BuildworkerChoiceParameter` has been added to provide a way to explicitly choose a buildworker
  for a given build.

* default.css now wraps preformatted text by default.

* Workers can now be paused through the web status.

* The latent buildworker support is less buggy, thanks :pull:`646`.
...
* A new :bb:step:`SetProperty` step has been added in ``buildbot.steps.master`` which can set a property directly without accessing the worker.
...
* OpenStack-based Latent Buildworker support was added.
  See :pull:`666`.

* Master-side support for P4 is available, and provides a great deal more flexibility than the old worker-side step.
  See :pull:`596`.
...
* The EC2 and libvirt latent workers have been moved to ``buildbot.buildworker.ec2`` and ``buildbot.buildworker.libirt`` respectively.
...
* The fix for Twisted bug #5079 is now applied on the worker side, too.
...
* The ``--nodaemon`` option to ``buildworker start`` now correctly prevents the worker from forking before running.
