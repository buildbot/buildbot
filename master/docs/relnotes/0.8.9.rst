...
* The following optional parameters have been added to :py:class:`EC2LatentBuildWorker`
  ...
  With ``spot_instance=True``, an ``EC2LatentBuildWorker`` will attempt to create a spot instance with the provided spot
...
* Information about the buildworkers (admin, host, etc) is now persisted in the database and available even if
  the worker is not connected.

* Buildworker info can now be retrieved via :ref:`Interpolate` and a new ``SetWorkerInfo`` buildstep.
...
* Builds are now retried after a worker is lost (:pull:`1049`).
...
* RemoteShellCommands can be killed by SIGTERM with the sigtermTime parameter before resorting to SIGKILL (:bug:`751`).
  If the worker's version is less than 0.8.9, the worker will kill the process with SIGKILL regardless of whether sigtermTime
  is supplied.
...
* The Perforce source step uses the correct path separator when the master is on Windows and the build worker is on a POSIX OS (:pull:`1114`).
...
* Fixed a race condition in worker shutdown (:pull:`1019`).
...
* The ``EC2LatentBuildWorker`` will now only consider available AMI's.

* Fixes a case where the first build runs on an old worker instead of a new one after reconfig (:bug:`2507`).
...
* Fixes a compatibility issue with HPCloud in the OpenStack latent worker.
...
* The worker-side source steps are deprecated in this version of Buildbot, and master-side support will be removed in a future version.
  Please convert any use of worker-side steps (imported directly from ``buildbot.steps.source``, rather than from a specific module like ``buildbot.steps.source.svn``) to use master-side steps.
...
* ``workerPortnum`` option deprecated, please use ``c['protocols']['pb']['port']`` to set up PB port
...
* The :py:class:`CompositeStepMixin` now provides a ``runGlob`` method to check for files on the worker that match a given shell-style pattern.
...
* Added zsh and bash tab-completions support for 'buildworker' command.
...
* Added a new remote command :py:class:`GlobPath` that can be used to call Python's ``glob.glob`` on the worker.
