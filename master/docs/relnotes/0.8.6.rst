...
* Worker properties in the configuration are now handled correctly.
...
* If a worker is lost while waiting for locks, it is properly cleaned up -
  :bug:`2247`.
...
* The :src:`master/contrib/fakemaster.py`` script allows you to run arbitrary commands on a worker by emulating a master.
