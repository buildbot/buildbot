...
* Fix :class:`DockerLatentWorker` use of `volume` parameter, they now properly manage `src:dest` syntax.
...
As the work is not completely finished, neither `buildbot-slave==0.9.0b8` or `buildbot-worker==0.9.0b8` have been released.
