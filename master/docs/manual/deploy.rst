...
If you find you can no longer provide a buildworker to the project, please let the project admins know, so they can put out a call for a replacement.
...
Alternatively, you can configure a maximum number of old logs to be kept using the ``--log-count`` command line option when running ``buildworker create-worker`` or ``buildbot create-master``.
...
.. _Starting-the-buildworker:

Starting the buildworker
~~~~~~~~~~~~~~~~~~~~~~~~
...
Otherwise, things will work fine for a while, but the first power outage or system reboot you have will stop the buildworker with nothing but the cries of sorrowful developers to remind you that it has gone away.
...
If the buildworker cannot connect to the buildmaster, the reason should be described in the :file:`twistd.log` logfile.
Some common problems are an incorrect master hostname or port number, or a mistyped bot name or password.
If the buildworker loses the connection to the master, it is supposed to attempt to reconnect with an exponentially-increasing backoff.
Each attempt (and the time of the next attempt) will be logged.
If you get impatient, just manually stop and re-start the buildworker.
...
Again, :samp:`buildworker restart {BASEDIR}` will speed up the process.
