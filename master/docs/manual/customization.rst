...
Writing a New Latent Buildworker Implementation
-----------------------------------------------

Writing a new latent buildworker should only require subclassing :class:`buildbot.buildworker.AbstractLatentBuildWorker` and implementing :meth:`start_instance` and :meth:`stop_instance`.
...
See :class:`buildbot.buildworker.ec2.EC2LatentBuildWorker` for an example.
...
.. code-block:: none
    ...
    Repo1 => <buildworker-base>/mybuilder/a78890ba
    Repo2 => <buildworker-base>/mybuilder/0823ba88
...
To spawn a command in the buildworker, create a :class:`~buildbot.process.remotecommand.RemoteCommand` instance in your step's ``run`` method and run it with :meth:`~buildbot.process.remotecommand.BuildStep.runCommand`::
...
For the much more common task of running a shell command on the buildworker, use :py:class:`~buildbot.process.buildstep.ShellMixin`.
...
Most commonly, logfiles come from commands run on the build worker.
...
The ``logfiles=`` argument to :bb:step:`ShellCommand` and its subclasses creates new log files and fills them in realtime by asking the buildworker to watch a actual file on disk.
The buildworker will look for additions in the target file and report them back to the :class:`BuildStep`.
    ...
    class MyBuildStep(steps.BuildStep):
        ...
        def start(self):
            ...
            if not all(workerver):
                raise BuildWorkerToOldError('need stat and glob')
