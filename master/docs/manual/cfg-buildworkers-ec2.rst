...
Using the AWS EC2 latent buildworkers involves getting an EC2 account with AWS and setting up payment; customizing one or more EC2 machine images ("AMIs") on your desired operating system(s) and publishing them (privately if needed); and configuring the buildbot master to know how to start your customized images for "substantiating" your latent workers.

This document will guide you through setup of a AWS EC2 latent buildworker:
...
To start off, to use the AWS EC2 latent buildworker, you need to get an AWS developer account and sign up for EC2.
...
* When an instance of the image starts, it needs to automatically start a buildbot worker that connects to your master (to create a buildbot worker, :ref:`Creating-a-buildworker`; to make a daemon, :ref:`Launching-the-daemons`).
* You may want to make an instance of the buildbot worker, configure it as a standard buildworker in the master (i.e., not as a latent worker), and test and debug it that way before you turn it into an AMI and convert to a latent worker in the master.

Configure the Master with an EC2LatentBuildWorker
-------------------------------------------------

Now let's assume you have an AMI that should work with the EC2LatentBuildWorker.
...
When creating an EC2LatentBuildWorker in the buildbot master configuration, the first three arguments are required.
The name and password are the first two arguments, and work the same as with normal buildworkers.
...
Here is the simplest example of configuring an EC2 latent buildworker.
...
::
    ...
    c['workers'] = [
        buildlsave.EC2LatentBuildWorker('bot1', 'sekrit', 'm1.large',
                                       ami='ami-12345',
...
Then you can instantiate the build worker as follows.

::

    from buildbot.plugins import buildworker
    c['workers'] = [
        buildworker.EC2LatentBuildWorker('bot1', 'sekrit', 'm1.large',
                                       ami='ami-12345')
    ]
...
::
    ...
    from buildbot.plugins import buildworker
    bot1 = buildworker.EC2LatentBuildWorker('bot1', 'sekrit', 'm1.large',
...
::
    ...
    from buildbot.plugins import buildworker
    bot1 = buildworker.EC2LatentBuildWorker(
...
The :class:`EC2LatentBuildWorker` supports all other configuration from the standard :class:`BuildWorker`.
...
They both default to ``latent_buildbot_worker``.

Spot instances
--------------

If you would prefer to use spot instances for running your builds, you can accomplish that by passing in a True value to the ``spot_instance`` parameter to the EC2LatentBuildWorker constructor.
...
If the request succeeds, the buildworker will substantiate as normal and run any pending builds.
