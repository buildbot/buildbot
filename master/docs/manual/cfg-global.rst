...
* You can have large numbers of build workers handling the same queue of build requests.
...
The buildmaster will listen on a TCP port of your choosing for connections from buildworkers.
It can also use this port for connections from remote Change Sources, status clients, and debug tools.
This port should be visible to the outside world, and you'll need to tell your buildworker admins about your choice.
...
This might be useful if you only run buildworkers on the same machine, and they are all configured to contact the buildmaster at ``localhost:10000``.

.. note::

   In Buildbot versions <=0.8.8 you might see ``workerPortnum`` option.
...
`manhole.TelnetManhole`
    ...
    Because this username/password is transmitted in the clear, and because Manhole access to the buildmaster is equivalent to granting full shell privileges to both the buildmaster and all the buildworkers (and to all accounts which then run code produced by the buildworkers), it is  highly recommended that you use one of the SSH manholes instead.
