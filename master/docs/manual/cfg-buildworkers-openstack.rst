...
This document will guide you through setup of an OpenStack latent buildworker:
...
Configure the Master with an OpenStackLatentBuildWorker
-------------------------------------------------------
...
:class:`OpenStackLatentBuildWorker` accepts the following arguments:

``name``
    The buildworker name.

``password``
    A password for the buildworker to login to the master with.
...
Here is the simplest example of configuring an OpenStack latent buildworker.

::

    from buildbot.plugins import buildworker
    c['workers'] = [
        buildworker.OpenStackLatentBuildWorker('bot2', 'sekrit',
...
:class:`OpenStackLatentBuildWorker` supports all other configuration from the standard :class:`BuildWorker`.
