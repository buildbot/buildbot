...
``oauth_key``
    OAuth key from bitbucket cloud.

``oauth_secret``
    OAuth secret from bitbucket cloud.

``repository_type``
   Set which type of repository link will be in the `repository` property. Possible values ``https``or ``ssh``. This link can then be used in a Source Step to checkout the source.
