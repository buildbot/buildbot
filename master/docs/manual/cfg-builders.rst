...
``workernames``
    These arguments specify the buildworker or buildworkers that will be used by this Builder.
    All workers names must appear in the :bb:cfg:`workers` configuration parameter.
    Each buildworker can accommodate multiple builders.
...
``workerbuilddir``
    Specifies the name of a subdirectory (under the worker's configured base directory) in which everything related to this builder will be placed on the buildworker.
...
``tags``
    ...
    A common use for this is to add new builders to your setup (for a new module, or for a new buildworker) that do not work correctly yet and allow you to integrate them with the active builders.
...
``nextWorker``
     ...
     The function is passed three arguments, the :class:`Builder` object which is assigning a new job, a list of :class:`WorkerBuilder` objects and the :class:`BuildRequest`.
     The function should return one of the :class:`WorkerBuilder` objects, or ``None`` if none of the available workers should be used.
     As an example, for each ``worker`` in the list, ``worker.worker`` will be a :class:`BuildWorker` object, and ``worker.worker.workername`` is the worker's name.
...
``canStartBuild``
    If provided, this is a function that can veto whether a particular buildworker should be used for a given build request.
    The function is passed three arguments: the :class:`Builder`, a :class:`BuildWorker`, and a :class:`BuildRequest`.
...
``env``
    ...
    The environment variables will override anything in the buildworker's environment.
