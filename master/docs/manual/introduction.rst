...
The buildworkers are designed to require an absolute minimum of configuration, reducing the effort a potential volunteer needs to expend to be able to contribute a new test environment to the project.
The goal is for anyone who wishes that a given project would run on their favorite platform should be able to offer that project a buildworker, running on that platform, where they can verify that their portability code works, and keeps working.
...
The Buildbot consists of a single *buildmaster* and one or more *buildworkers*, connected in a star topology.
The buildmaster makes all decisions about what, when, and how to build.
It sends commands to be run on the build workers, which simply execute the commands and return the results.
...
Each buildworker is maintained by a *buildworker admin*, who do not need to be quite as involved.
Generally workers are run by anyone who has an interest in seeing the project work well on their favorite platform.

.. _BuildWorker-Connections:

BuildWorker Connections
~~~~~~~~~~~~~~~~~~~~~~~

The buildworkers are typically run on a variety of separate machines, at least one per platform of interest.
These machines connect to the buildmaster over a TCP connection to a publically-visible port.
As a result, the buildworkers can live behind a NAT box or similar firewalls, as long as they can get to buildmaster.
The TCP connections are initiated by the buildworker and accepted by the buildmaster, but commands and results travel both ways within this connection.
The buildmaster is always in charge, so all commands travel exclusively from the buildmaster to the buildworker.

To perform builds, the buildworkers must typically obtain source code from a CVS/SVN/etc repository.
...
Schedulers
    ...
    They collect :class:`Change`\s into :class:`BuildRequest`\s, which are then queued for delivery to :class:`Builders` until a buildworker is available.

Builders
    Which control exactly *how* each build is performed (with a series of :class:`BuildStep`\s, configured in a :class:`BuildFactory`).
    Each :class:`Build` is run on a single buildworker.

Status plugins
    Which deliver information about the build results through protocols like HTTP, mail, and IRC.

Each :class:`Builder` is configured with a list of :class:`BuildWorker`\s that it will use for its builds.
These buildworkers are expected to behave identically: the only reason to use multiple :class:`BuildWorker`\s for a single :class:`Builder` is to provide a measure of load-balancing.

Within a single :class:`BuildWorker`, each :class:`Builder` creates its own :class:`WorkerBuilder` instance.
These :class:`WorkerBuilder`\s operate independently from each other.
Each gets its own base directory to work in.
It is quite common to have many :class:`Builder`\s sharing the same buildworker.
For example, there might be two buildworkers: one for i386, and a second for PowerPC.
...
The full builders would each run on a single buildworker, whereas the tarball creation step might run on either buildworker (since the platform doesn't matter when creating source tarballs).
In this case, the mapping would look like:

.. code-block:: none

    Builder(full-i386)  ->  BuildWorkers(worker-i386)
    Builder(full-ppc)   ->  BuildWorkers(worker-ppc)
    Builder(source-tarball) -> BuildWorkers(worker-i386, worker-ppc)

and each :class:`BuildWorker` would have two :class:`WorkerBuilders` inside it, one for a full builder, and a second for the source-tarball builder.

Once a :class:`WorkerBuilder` is available, the :class:`Builder` pulls one or more :class:`BuildRequest`\s off its incoming queue.
...
The :class:`Build` is then randomly assigned to a free :class:`WorkerBuilder` and the build begins.
...
There are also status objects that correspond to :class:`Scheduler`\s and :class:`BuildWorker`\s.
These allow status plugins to report information about upcoming builds, and the online/offline status of each buildworker.
...
* The buildmaster distributes this change to all of its configured schedulers.
  ...
  Unless configured otherwise, all :class:`Build`\s run in parallel on the various buildworkers.

* The :class:`Build` consists of a series of :class:`Step`\s.
  Each :class:`Step` causes some number of commands to be invoked on the remote buildworker associated with that :class:`Builder`.
  ...
  As each :class:`Step` runs, the buildworker reports back command output and return status to the buildmaster.
