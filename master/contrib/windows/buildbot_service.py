...
def _RunChild(runfn):
    ...
    # Start the buildbot/buildworker app
    runfn()
...
def DetermineRunner(bbdir):
    '''Checks if the given directory is a buildworker or a master and returns the
    appropriate run function.'''
    try:
        import buildworker.scripts.runner
        tacfile = os.path.join(bbdir, 'buildbot.tac')

        if os.path.exists(tacfile):
            with open(tacfile, 'r') as f:
                contents = f.read()
                if 'import BuildWorker' in contents:
                    return buildworker.scripts.runner.run
