...
with e.g. 'apt-get install python-gitlab' or 'pip install pyapi-gitlab'.
...
Note that this gateway only handles merge request events, not push events.
To handle push events, pass "change_hook_dialects={ 'gitlab' : True }"
to html.WebStatus() in your master.cfg.
See http://docs.buildbot.net/latest/manual/cfg-wwwhooks.html#gitlab-hook
You should probably get that working first if you haven't already.

Assumes that you've set up buildbot to accept PB style try builds, and
that 'buildbot try -c pb --username=XXXXX --passwd=YYYYY ...' already works.
...
See http://docs.buildbot.net/latest/tutorial/tour.html#adding-a-try-scheduler

It also assumes that your try builders have names that match the
regular expression projname.*branch, e.g. if the project name in
gitlab is wurlitzer, and the merge request targets the master branch,
the gateway will use try builders wurlitzer-macosx-master-snorkle and
wurlitzer-master, but not wurlitzer-release.  You may need to adjust
the select_builders function in this file if your naming convention
differs.

In a gitlab project's settings page, click on Web Hooks, uncheck Push
Events, check Merge Request events, and enter hostname:9999 in the URL
field, where hostname is the machine that will run this script.
...
# On Ubuntu 12.04, if you install Requests via apt-get install python-requests,
# you'll need to modify pyapi-gitlab slightly to run against that old version.
...
# slightly to not pass the verify parameters that old Requests doesn't
# support).
...
def verbose_system(git, project_id, mr_id, s):
    """Output error to stdout and gitlab and raise exception if command failed"""
    print(s)
    sys.stdout.flush()
    try:
...
def select_builders(git, project_id, mr_id, try_user, try_pass, try_server, project, ubranch):
    """Return list of try builder names to use for this project/branch.
    On error, complain to gitlab merge request and to stdout, and return None.
    """
...
def do_try(project, comment, surl, sbranch, uurl, ubranch, who, project_id, mr_id):
    ...
    sys.stdout.flush()
    secrets = json.load(
        open(os.path.expanduser("~/gitlab_buildbot.json")))
    ...
    builders = select_builders(git, project_id, mr_id, try_user, try_pass, try_server, project, ubranch)
    if (builders is None):
        print("no builders?")
        sys.stdout.flush()
        return

    opts = " ".join(["-b " + x for x in builders])
    dirpath = tempfile.mkdtemp()

    try:
        verbose_system(git, project_id, mr_id, "git clone -b %s %s %s" %
                       (ubranch, uurl, dirpath))
        verbose_system(git, project_id, mr_id,
                       "cd %s && git remote add downstream %s && git fetch downstream" % (dirpath, surl))
        verbose_system(git, project_id, mr_id, "cd %s && git checkout -b branch-to-try--%s downstream/%s" %
                       (dirpath, sbranch, sbranch))
        verbose_system(git, project_id, mr_id, "cd %s && git format-patch -M -B origin/%s --stdout > change.patch" %
                       (dirpath, ubranch))
    except:
        # error has already been sent to stdout and gitlab
        return

    cmd = "buildbot try --wait -c pb --username=%s --who='%s' --passwd=%s -m %s --diff=change.patch --patchlevel 1 %s --comment '%s'" % (
        try_user, my_quote(who), try_pass, try_server, opts, my_quote(comment))
    ...
    try:
        ...
        # loop reading it and submitting comments while waiting for it to
        # finish.
    ...
    f = open("log.txt", "r")
    log = f.read()
    # Remove non-interesting lines. (?s) means 'dot matches newline too'.
    ...
    htmlmsg = re.sub(
        r'(\S*): (.*\[)build (\d+)', r'\1: \2<a href="%s/builders/\1/builds/\3">build \3</a>' % web_status, htmlmsg)
    # Turn builder names into hyperlinks.
    htmlmsg = re.sub(r'(\S*): (shell|failure|success)',
                     r'<a href="%s/builders/\1">\1</a>: \2' % web_status, htmlmsg)
...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if postvars == {} or 'object_kind' not in postvars:
            ...
            self.reply_fail(400, reason='cannot handle object_kind=%s' %
                            postvars['object_kind'])


def run(server_class=ForkingHTTPServer, handler_class=RequestHandler, port=9999):
    server_address = ('', port)
    print 'gitlab_buildbot listening on port %d...' % port
    sys.stdout.flush()
...
if __name__ == "__main__":
    ...
    if len(argv) == 10:
        # for debugging only
        # project, comment, surl, sbranch,
        # uurl, ubranch, who, project_id, mr_id
        do_try(argv[1], argv[2], argv[3], argv[4],
