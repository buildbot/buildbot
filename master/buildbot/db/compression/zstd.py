...
class ZStdCompressor(CompressorInterface):
    ...
    def read(data: bytes) -> bytes:
        # not too sure why, but zstandard.decompress do not work with
        # data compressed with `ZstdCompressionObj`
        # `ZstdDecompressionObj` does and can also decompress data from `zstandard.compress`
