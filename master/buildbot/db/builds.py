    ...
    def getBuildsForChange(self, changeid):
        ...
        def thd(conn):
            ...
            from_clause = from_clause.join(
                builds_tbl, reqs_tbl.c.id == builds_tbl.c.buildrequestid
            )
        ...
        def thd(conn):
            ...
            while True:
                ...
                try:
                    ...
                    # numbers
                    if 'duplicate key value violates unique constraint "builds_pkey"' not in str(
                        e
                    ):
                        new_number += 1
                    continue
    ...
    def setBuildProperty(self, bid, name, value, source):
        ...
        def thd(conn):
            ...
            if prop is None:
                conn.execute(
                    bp_tbl.insert(),
                    dict(buildid=bid, name=name, value=value_js, source=source),
                )
            elif (prop.value != value_js) or (prop.source != source):
                conn.execute(
                    bp_tbl.update(whereclause=whereclause),
