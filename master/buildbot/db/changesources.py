...
class ChangeSourcesConnectorComponent(base.DBConnectorComponent):
    ...
    def setChangeSourceMaster(self, changesourceid, masterid):
        def thd(conn):
            ...
            try:
                q = cs_mst_tbl.insert()
                conn.execute(q, dict(changesourceid=changesourceid, masterid=masterid))
    ...
    def getChangeSources(self, active=None, masterid=None, _changesourceid=None):
        def thd(conn):
            ...
            q = sa.select(
                [cs_tbl.c.id, cs_tbl.c.name, cs_mst_tbl.c.masterid],
                from_obj=join,
                whereclause=wc,
            )

            return [
                dict(id=row.id, name=row.name, masterid=row.masterid)
