...
from buildbot.process.workerbuilder import BUILDING
...
class Builder(util_service.ReconfigurableServiceMixin,
    ...
    def __init__(self, name, _addServices=True):
        ...
        # buildworkers which have connected but which are not yet available.
        # These are always in the ATTACHING state.
        self.attaching_workers = []

        # buildworkers at our disposal. Each WorkerBuilder instance has a
    ...
    def reconfigServiceWithBuildbotConfig(self, new_config):
        ...
        # if we have any workerbuilders attached which are no longer configured,
        # drop them.
        new_workernames = set(builder_config.workernames)
        self.workers = [s for s in self.workers
                       if s.worker.workername in new_workernames]
    ...
    def addLatentWorker(self, worker):
        assert interfaces.ILatentBuildWorker.providedBy(worker)
        for s in self.workers:
            if s == worker:
                break
        else:
            sb = workerbuilder.LatentWorkerBuilder(worker, self)
    ...
    def attached(self, worker, commands):
        """This is invoked by the BuildWorker when the self.workername bot
        registers their builder.

        @type  worker: L{buildbot.buildworker.BuildWorker}
        @param worker: the BuildWorker that represents the buildworker as a whole
        ...
        """
        for s in self.attaching_workers + self.workers:
            if s.worker == worker:
        ...
        sb = workerbuilder.WorkerBuilder()
        sb.setBuilder(self)
    ...
    def _not_attached(self, why, worker):
        # already log.err'ed by WorkerBuilder._attachFailure
    ...
    def detached(self, worker):
        ...
        sb.detached()  # inform the WorkerBuilder that their worker went away
        self.updateBigStatus()
    ...
    def canStartBuild(self, workerbuilder, breq):
        if callable(self.config.canStartBuild):
            return defer.maybeDeferred(self.config.canStartBuild, self, workerbuilder, breq)
        return defer.succeed(True)

    @defer.inlineCallbacks
    def _startBuildFor(self, workerbuilder, buildrequests):
        ...
        log.msg("starting build %s using worker %s" % (build, workerbuilder))

        # set up locks
        build.setLocks(self.config.locks)
        cleanups.append(workerbuilder.worker.releaseLocks)
        ...
        try:
            ready = yield workerbuilder.prepare(self.builder_status, build)
        except Exception:
            log.err(failure.Failure(), 'while preparing workerbuilder:')
            ready = False
        ...
        log.msg("starting build %s.. pinging the worker %s"
                % (build, workerbuilder))
        try:
            ping_success = yield workerbuilder.ping()
        except Exception:
        ...
        # The buildworker is ready to go. workerbuilder.buildStarted() sets its
        ...
        cleanups.append(lambda: workerbuilder.buildFinished())

        # tell the remote that it's starting a build, too
        try:
            yield workerbuilder.worker.conn.remoteStartBuild(build.builder.name)
        except Exception:
        ...
        if not workerbuilder.worker.conn:  # TODO: replace with isConnected()
        ...
        d = defer.maybeDeferred(build.startBuild,
                                bs, self.expectations, workerbuilder)
        d.addCallback(lambda _: self.buildFinished(build, workerbuilder))
    ...
    @defer.inlineCallbacks
    def maybeStartBuild(self, workerbuilder, breqs, _reactor=reactor):
        ...
        build_started = yield self._startBuildFor(workerbuilder, breqs)
...
class BuilderControl:
    ...
    def ping(self):
        ...
        dl = []
        for s in self.original.workers:
