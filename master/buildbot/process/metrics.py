...
class AttachedWorkersWatcher(object):
    ...
    def run(self):
        ...
        # 'AbstractBuildWorker.attached_workers'
        ...
        buildworker_count = h.get('AbstractBuildWorker.attached_workers')

        # We let these be off by one since they're counted at slightly
        # different times
        if abs(botmaster_count - buildworker_count) > 1:
            level = ALARM_WARN
        else:
            level = ALARM_OK

        MetricAlarmEvent.log('attached_workers',
                             msg='%s %s' % (botmaster_count, buildworker_count),
                             level=level)
