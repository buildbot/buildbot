...
class BuildRequest:
    ...
    def canBeCollapsed(master, new_br, old_br):
        ...
        new_bs_props = yield master.data.get(
            ("buildsets", str(new_br["buildsetid"]), "properties")
        )
        old_bs_props = yield master.data.get(
            ("buildsets", str(old_br["buildsetid"]), "properties")
        )
