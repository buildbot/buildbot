...
class BuildChooserBase(object):
    ...
    #
    # The entry point is:
    #    * bc.chooseNextBuild() - get the next (worker, [breqs]) or (None, None)
    #
...
class BasicBuildChooser(BuildChooserBase):
    ...
    def _popNextWorker(self, buildrequest):
        ...
        while self.workerpool:
            ...
            canStart = yield self.bldr.canStartWithWorkerbuilder(worker)
            if canStart:
                defer.returnValue(worker)
                return
    ...
    def _maybeStartBuildsOnBuilder(self, bldr, _reactor=reactor):
        ...
        while True:
            ...
            if not buildStarted:
                ...
                # and try starting builds again.  If we still have a working worker,
