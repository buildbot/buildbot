...
class SkipWorkersThatCantGetLock(buildrequestdistributor.BasicBuildChooser):
...
class TestBRDBase(unittest.TestCase):
    ...
    def addWorkers(self, workerbuilders):
        """C{workers} maps name : available"""
        for name, avail in iteritems(workerbuilders):
    ...
    def createBuilder(self, name, builderid=None):
        ...
        bldr.canStartWithWorkerbuilder = lambda _: True
...
class TestMaybeStartBuilds(TestBRDBase):
    ...
    @defer.inlineCallbacks
    def test_no_workerbuilders(self):
        rows = [
    ...
    def test_limited_by_canStartBuild(self):
        ...
        def _canStartWithWorkerbuilder(workerbuilder):
            workers_attempted.append(workerbuilder.name)
            return True
        self.bldr.canStartWithWorkerbuilder = _canStartWithWorkerbuilder
    ...
    @mock.patch('buildbot.process.buildrequestdistributor.BuildRequestDistributor.BuildChooser', SkipWorkersThatCantGetLock)
    @defer.inlineCallbacks
    def test_limited_by_canStartBuild_deferreds(self):
        """Another variant that:
         * returns Deferred types,
         * use 'canStartWithWorkerbuilder' to reject one of the workers
         * patch using SkipWorkersThatCantGetLock to disable the 'rejectedWorkers' feature"""
        ...
        def _canStartWithWorkerbuilder(workerbuilder):
            workers_attempted.append(workerbuilder.name)
            allowed = workerbuilder.name in ['test-worker2', 'test-worker1']
    ...
    @defer.inlineCallbacks
    def test_limited_by_canStartWithWorkerbuilder(self):
        self.bldr.config.nextWorker = nth_worker(-1)
        ...
        def _canStartWithWorkerbuilder(workerbuilder):
            workers_attempted.append(workerbuilder.name)
            return (workerbuilder.name == 'test-worker3')
        self.bldr.canStartWithWorkerbuilder = _canStartWithWorkerbuilder
    ...
    def do_test_nextWorker(self, nextWorker, exp_choice=None):
        ...
        self.bldr.config.nextWorker = nextWorker
        rows = self.base_rows + [
            fakedb.BuildRequest(id=11, buildsetid=11, builderid=77),
        ]
        ...
        if exp_choice is None:
            ...
            exp_claims = [11]
            exp_builds = [('test-worker%d' % exp_choice, [11])]
