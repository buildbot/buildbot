...
class Sender(unittest.TestCase):
    ...
    def test_send_full_backward_compatibility(self):
        s = sendchange.Sender('localhost:1234')
        ...
        def check(_):
            self.assertProcess('localhost', 1234, 'change', 'changepw', [
                dict(project='p', repository='r', who='me', files=['a'],
                    ...
                    revlink='rl', src='git', tags=None)])
        d.addCallback(check)
        return d

    def test_send_full(self):
        s = sendchange.Sender('localhost:1234')
        d = s.send('branch', 'rev', 'comm', ['a'], who='me',
                   when=1234, properties={'a':'b'}, repository='r', vc='git',
                   project='p', revlink='rl',  tags=['cats','dogs'])
        def check(_):
            self.assertProcess('localhost', 1234, 'change', 'changepw', [
                dict(project='p', repository='r', who='me', files=['a'],
                    comments='comm', branch='branch', revision='rev',
                    category=None, when=1234, properties={'a':'b'},
                    revlink='rl', src='git', tags=['cats','dogs'])])
        d.addCallback(check)
        return d
    ...
    def test_send_unicode_backward_compatibility(self):
        s = sendchange.Sender('localhost:1234')
        ...
        def check(_):
            self.assertProcess('localhost', 1234, 'change', 'changepw', [
                dict(project=u'\N{SKULL AND CROSSBONES}',
                     ...
                     properties={u'\N{LATIN SMALL LETTER A WITH MACRON}':'b'},
                     revlink=u'\U0001F517', # LINK SYMBOL
                     src=None, category=None)])
        d.addCallback(check)
        return d

    def test_send_unicode_utf8_backward_compatibility(self):
        s = sendchange.Sender('localhost:1234')
    ...
    def test_send_unicode_latin1_backward_compatibility(self):
