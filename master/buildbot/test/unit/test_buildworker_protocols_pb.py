...
from buildbot.buildworker.protocols import pb
...
class TestListener(unittest.TestCase):
    ...
    def test_getPerspective(self):
        listener = self.makeListener()
        buildworker = mock.Mock()
        buildworker.workername = 'test'
        mind = mock.Mock()

        listener.updateRegistration('example', 'pass', 'tcp:1234')
        self.master.buildworkers.register(buildworker)
        conn = yield listener._getPerspective(mind, buildworker.workername)
...
class TestConnection(unittest.TestCase):
    ...
    def setUp(self):
        self.master = fakemaster.make_master()
        self.mind = mock.Mock()
        self.buildworker = mock.Mock()

    def test_constructor(self):
        conn = pb.Connection(self.master, self.buildworker, self.mind)

        self.assertEqual(conn.mind, self.mind)
        self.assertEqual(conn.master, self.master)
        self.assertEqual(conn.buildworker, self.buildworker)
    ...
    def test_attached(self):
        ...
        self.buildworker.attached.assert_called_with(conn)
    ...
    def test_remoteGetWorkerInfo(self):
        ...
        calls = [mock.call('getWorkerInfo'), mock.call('getCommands'), mock.call('getVersion')]
    ...
    def test_perspective_shutdown(self):
        ...
        conn.buildworker.shutdownRequested.assert_called_with()
        conn.buildworker.messageReceivedFromWorker.assert_called_with()
