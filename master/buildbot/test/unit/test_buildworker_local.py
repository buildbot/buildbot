...
from buildbot.buildworker import local
from buildbot.test.fake import fakemaster
from twisted.internet import defer
from twisted.trial import unittest


class TestLocalBuildWorker(unittest.TestCase):

    def setUp(self):
        self.master = fakemaster.make_master(wantDb=True, wantData=True,
                                             testcase=self)
        self.botmaster = self.master.botmaster
        self.buildworkers = self.master.buildworkers

    def createBuildworker(self, name='bot', attached=False, configured=True, **kwargs):
        worker = local.LocalBuildWorker(name, **kwargs)
        if configured:
    ...
    def test_reconfigService_attrs(self):
        old = self.createBuildworker('bot',
                                    max_builds=2,
                                    notify_on_missing=['me@me.com'],
                                    missing_timeout=120,
                                    properties={'a': 'b'})
        new = self.createBuildworker('bot', configured=False,
                                    max_builds=3,
