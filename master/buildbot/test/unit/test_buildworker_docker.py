...
from buildbot.buildworker import docker as dockerbuildworker
from buildbot.test.fake import docker
from twisted.internet import defer


class TestDockerLatentBuildWorker(unittest.TestCase):

    class ConcreteBuildWorker(dockerbuildworker.DockerLatentBuildWorker):
        pass

    def setUp(self):
        self.patch(dockerbuildworker, 'client', docker)

    def test_constructor_nodocker(self):
        self.patch(dockerbuildworker, 'client', None)
        self.assertRaises(config.ConfigErrors, self.ConcreteBuildWorker, 'bot', 'pass', 'unix://tmp.sock', 'debian:wheezy', [])

    def test_constructor_noimage_nodockerfile(self):
        self.assertRaises(config.ConfigErrors, self.ConcreteBuildWorker, 'bot', 'pass', 'http://localhost:2375')

    def test_constructor_noimage_dockerfile(self):
        bs = self.ConcreteBuildWorker('bot', 'pass', 'http://localhost:2375', dockerfile="FROM ubuntu")
        self.assertEqual(bs.dockerfile, "FROM ubuntu")
        self.assertEqual(bs.image, None)

    def test_constructor_image_nodockerfile(self):
        bs = self.ConcreteBuildWorker('bot', 'pass', 'http://localhost:2375', image="myworker")
        self.assertEqual(bs.dockerfile, None)
        self.assertEqual(bs.image, 'myworker')

    def test_constructor_minimal(self):
        # Minimal set of parameters
        bs = self.ConcreteBuildWorker('bot', 'pass', 'tcp://1234:2375', 'worker')
    ...
    def test_constructor_all_docker_parameters(self):
        ...
        bs = self.ConcreteBuildWorker('bot', 'pass', 'unix:///var/run/docker.sock', 'worker_img', ['/bin/sh'], dockerfile="FROM ubuntu", version='1.9', tls=True)
        self.assertEqual(bs.workername, 'bot')
        self.assertEqual(bs.password, 'pass')
    ...
    def test_volume_no_suffix(self):
        bs = self.ConcreteBuildWorker('bot', 'pass', 'tcp://1234:2375', 'worker', ['bin/bash'], volumes=['/src/webapp:/opt/webapp'])
    ...
    def test_ro_rw_volume(self):
        bs = self.ConcreteBuildWorker('bot', 'pass', 'tcp://1234:2375', 'worker', ['bin/bash'],
    ...
    def test_volume_bad_format(self):
        self.assertRaises(config.ConfigErrors, self.ConcreteBuildWorker, 'bot', 'pass', 'http://localhost:2375', image="worker",
                          volumes=['abcd=efgh'])

    @defer.inlineCallbacks
    def test_start_instance_image_no_version(self):
        bs = self.ConcreteBuildWorker('bot', 'pass', 'tcp://1234:2375', 'busybox', ['bin/bash'])
        id, name = yield bs.start_instance(None)
        self.assertEqual(name, 'busybox')

    @defer.inlineCallbacks
    def test_start_instance_image_right_version(self):
        bs = self.ConcreteBuildWorker('bot', 'pass', 'tcp://1234:2375', 'busybox:latest', ['bin/bash'])
        id, name = yield bs.start_instance(None)
        self.assertEqual(name, 'busybox:latest')

    @defer.inlineCallbacks
    def test_start_instance_image_wrong_version(self):
        bs = self.ConcreteBuildWorker('bot', 'pass', 'tcp://1234:2375', 'busybox:previous', ['bin/bash'])
        try:
            id, name = yield bs.start_instance(None)
        except interfaces.LatentBuildWorkerFailedToSubstantiate:
            pass
    ...
    def test_start_instance_noimage_dockefilefails(self):
        bs = self.ConcreteBuildWorker('bot', 'pass', 'tcp://1234:2375', 'worker', dockerfile='BUG')
        try:
            id, name = yield bs.start_instance(None)
        except interfaces.LatentBuildWorkerFailedToSubstantiate:
            pass
    ...
    def test_start_instance_noimage_gooddockerfile(self):
        bs = self.ConcreteBuildWorker('bot', 'pass', 'tcp://1234:2375', 'worker', dockerfile='FROM debian:wheezy')
        id, name = yield bs.start_instance(None)
...
class testDockerPyStreamLogs(unittest.TestCase):
    ...
    def compare(self, result, log):
        self.assertEquals(result,
                          list(dockerbuildworker.handle_stream_line(log)))
