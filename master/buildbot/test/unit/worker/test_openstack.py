...
class TestOpenStackWorker(TestReactorMixin, unittest.TestCase):
    ...
    os_auth_custom = dict(token="openstack-token", auth_type="token", auth_url="auth")

    bs_image_args = dict(flavor=1, image="image-uuid", **os_auth)
    ...
    def test_start_instance_check_meta_renderable(self):
        ...
        self.assertIn("meta", bs.instance.boot_kwargs)
        self.assertEquals(
            bs.instance.metadata,
            {"some_key": "value", "BUILDBOT:instance": self.masterhash},
        )
