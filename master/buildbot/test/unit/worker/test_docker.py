...
class TestDockerLatentWorker(ConfigErrorsMixin, unittest.TestCase, TestReactorMixin):
    ...
    async def test_constructor_noimage_nodockerfile(self):
        with self.assertRaises(config.ConfigErrors):
            await self.setupWorker('bot', 'pass', 'http://localhost:2375')

    async def test_constructor_noimage_dockerfile(self):
    ...
    async def test_constructor_image_nodockerfile(self):
        bs = await self.setupWorker('bot', 'pass', 'http://localhost:2375', image="myworker")
        self.assertEqual(bs.dockerfile, None)
        self.assertEqual(bs.image, 'myworker')

    async def test_constructor_minimal(self):
        # Minimal set of parameters
        bs = await self.setupWorker('bot', 'pass', 'tcp://1234:2375', 'worker')
    ...
    async def test_contruction_too_old_docker(self):
        ...
        with self.assertRaisesConfigError("The python module 'docker>=4.0'"):
            await self.setupWorker('bot', 'pass', 'tcp://1234:2375', 'worker')

    async def test_contruction_minimal_docker(self):
        bs = await self.setupWorker('bot', 'pass', 'tcp://1234:2375', 'worker')
        await bs.start_instance(self.build)
        self.assertEqual(
            [c["Names"] for c in self._client._containers.values()], [["/buildbot-bot-87de7e"]]
        )

    async def test_constructor_nopassword(self):
        # when no password, it is created automatically
        bs = await self.setupWorker('bot', None, 'tcp://1234:2375', 'worker')
        self.assertEqual(bs.workername, 'bot')
        self.assertEqual(len(bs.password), 20)

    async def test_constructor_all_docker_parameters(self):
    ...
    async def test_constructor_host_config_build(self):
        # Volumes have their own tests
        bs = await self.setupWorker(
            'bot',
            'pass',
    ...
    async def test_constructor_host_config_build_set_init(self):
        # Volumes have their own tests
        bs = await self.setupWorker(
            'bot',
            'pass',
            ...
            dockerfile="FROM ubuntu",
            volumes=["/tmp:/tmp:ro"],
            hostconfig={'network_mode': 'fake', 'dns': ['1.1.1.1', '1.2.3.4'], 'init': False},
            custom_context=False,
            buildargs=None,
            encoding='gzip',
        )
        await bs.start_instance(self.build)
        self.assertEqual(
            self._client.call_args_create_host_config,
            [
                {
                    'network_mode': 'fake',
                    'dns': ['1.1.1.1', '1.2.3.4'],
    ...
    async def test_start_instance_docker_host_renderable(self):
        bs = await self.setupWorker(
    ...
    async def test_start_instance_volume_renderable(self):
        bs = await self.setupWorker(
            'bot',
            'pass',
            'tcp://1234:2375',
            ...
            ['bin/bash'],
            volumes=[
                Interpolate('/data:/worker/%(kw:builder)s/build', builder=Property('builder'))
            ],
        )
        await bs.start_instance(self.build)
        self.assertEqual(len(self._client.call_args_create_container), 1)
        self.assertEqual(
            self._client.call_args_create_container[0]['volumes'], ['/worker/docker_worker/build']
        )

    async def test_start_instance_hostconfig_renderable(self):
        bs = await self.setupWorker(
            'bot',
            'pass',
            docker_host='tcp://1234:2375',
            image='worker',
            hostconfig={'prop': Interpolate('value-%(kw:builder)s', builder=Property('builder'))},
        )
        await bs.start_instance(self.build)
        self.assertEqual(len(self._client.call_args_create_container), 1)
    ...
    async def test_volume_no_suffix(self):
        bs = await self.setupWorker(
            'bot',
            'pass',
            'tcp://1234:2375',
        ...
        self.assertEqual(self._client.call_args_create_container[0]['volumes'], ['/opt/webapp'])
        self.assertEqual(
            self._client.call_args_create_host_config[0]['binds'], ["/src/webapp:/opt/webapp"]
        )

    async def test_volume_ro_rw(self):
        bs = await self.setupWorker(
            'bot',
            'pass',
            'tcp://1234:2375',
            'worker',
            ['bin/bash'],
            volumes=['/src/webapp:/opt/webapp:ro', '~:/backup:rw'],
        )
        await bs.start_instance(self.build)
        self.assertEqual(len(self._client.call_args_create_container), 1)
        self.assertEqual(len(self._client.call_args_create_host_config), 1)
        self.assertEqual(
            self._client.call_args_create_container[0]['volumes'], ['/opt/webapp', '/backup']
        )
        self.assertEqual(
            self._client.call_args_create_host_config[0]['binds'],
            ['/src/webapp:/opt/webapp:ro', '~:/backup:rw'],
        )

    async def test_volume_bad_format(self):
        with self.assertRaises(config.ConfigErrors):
            await self.setupWorker(
                'bot', 'pass', 'http://localhost:2375', image="worker", volumes=['abcd=efgh']
            )

    async def test_volume_bad_format_renderable(self):
        bs = await self.setupWorker(
            'bot',
            'pass',
            'http://localhost:2375',
    ...
    async def test_start_instance_image_no_version(self):
        bs = await self.setupWorker('bot', 'pass', 'tcp://1234:2375', 'busybox', ['bin/bash'])
        _, name = await bs.start_instance(self.build)
        self.assertEqual(name, 'busybox')

    async def test_start_instance_image_right_version(self):
        bs = await self.setupWorker(
    ...
    async def test_start_instance_image_wrong_version(self):
        bs = await self.setupWorker('bot', 'pass', 'tcp://1234:2375', 'busybox:123', ['bin/bash'])
        with self.assertRaises(interfaces.LatentWorkerCannotSubstantiate):
            await bs.start_instance(self.build)

    async def test_start_instance_image_renderable(self):
        bs = await self.setupWorker(
    ...
    async def test_start_instance_noimage_nodockerfile(self):
        bs = await self.setupWorker('bot', 'pass', 'tcp://1234:2375', 'customworker', ['bin/bash'])
        with self.assertRaises(interfaces.LatentWorkerCannotSubstantiate):
            await bs.start_instance(self.build)

    async def test_start_instance_image_and_dockefile(self):
        bs = await self.setupWorker(
    ...
    async def test_start_instance_noimage_gooddockerfile(self):
        bs = await self.setupWorker(
    ...
    async def test_start_instance_noimage_pull(self):
        bs = await self.setupWorker(
    ...
    async def test_start_instance_image_pull(self):
        bs = await self.setupWorker(
    ...
    async def test_start_instance_image_alwayspull(self):
        bs = await self.setupWorker(
    ...
    async def test_start_instance_image_noauto_alwayspull(self):
        bs = await self.setupWorker(
    ...
    async def test_start_instance_noimage_renderabledockerfile(self):
        bs = await self.setupWorker(
            'bot',
            'pass',
            'tcp://1234:2375',
            'customworker',
            dockerfile=Interpolate('FROM debian:%(kw:distro)s', distro=Property('distro')),
        )
        _, name = await bs.start_instance(self.build)
        self.assertEqual(name, 'customworker')

    async def test_start_instance_custom_context_and_buildargs(self):
        bs = await self.setupWorker(
            'bot',
            'pass',
            'tcp://1234:2375',
    ...
    async def test_start_instance_custom_context_no_buildargs(self):
        bs = await self.setupWorker(
            'bot',
            'pass',
            'tcp://1234:2375',
    ...
    async def test_start_instance_buildargs_no_custom_context(self):
        bs = await self.setupWorker(
            'bot',
            'pass',
            'tcp://1234:2375',
    ...
    async def test_start_worker_but_already_created_with_same_name(self):
        bs = await self.setupWorker(
    ...
    async def test_start_instance_client_start_exception(self):
    ...
    async def test_constructor_hostname(self):
        bs = await self.setupWorker(
            'bot',
            'pass',
            'http://localhost:2375',
            image="myworker_image",
            hostname="myworker_hostname",
        )
        self.assertEqual(bs.hostname, 'myworker_hostname')

    async def test_check_instance_running(self):
        bs = await self.setupWorker('bot', 'pass', 'tcp://1234:2375', 'worker')
        await bs.start_instance(self.build)
        self.assertEqual((yield bs.check_instance()), (True, ""))

    async def test_check_instance_exited(self):
        bs = await self.setupWorker('bot', 'pass', 'tcp://1234:2375', 'worker')
        await bs.start_instance(self.build)
