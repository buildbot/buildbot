...
class TestSendChangeOptions(OptionsMixin, unittest.TestCase):
    ...
    def test_defaults(self):
        ...
        exp = dict(master='m', auth=('change', 'changepw'), who='me', vc=None,
                ...
                tags=[], revision=None, revision_file=None, property=None,
    ...
    def test_tags(self):
        opts = self.parse('-ttag', *self.master_and_who)
        self.assertOptions(opts, dict(tags=['tag']))

    def test_tags_long(self):
        opts = self.parse('--tags=tag', *self.master_and_who)
        self.assertOptions(opts, dict(tags=['tag']))

    def test_tags_multiple(self):
        opts = self.parse('-tcat', '-tdog', *self.master_and_who)
        self.assertOptions(opts, dict(tags=['cat', 'dog']))

    def test_tags_long_multiple(self):
        opts = self.parse('--tags=cat', '--tags=dog', *self.master_and_who)
        self.assertOptions(opts, dict(tags=['cat', 'dog']))

    def test_tags_token(self):
        opts = self.parse('-t', 'tag',  *self.master_and_who)
        self.assertOptions(opts, dict(tags=['tag']))

    def test_config_tagss(self):
        self.options_file['master'] = 'MMM'
        self.options_file['who'] = 'WWW'
        self.options_file['tags'] = ['a', 'b']
        opts = self.parse()
        self.assertOptions(opts, dict(tags=['a', 'b']))

    def test_config_tagss_override(self):
        self.options_file['master'] = 'MMM'
        self.options_file['who'] = 'WWW'
        self.options_file['tags'] = ['a', 'b']
        opts = self.parse('-t', 'd') # overrides a, b
        self.assertOptions(opts, dict(tags=['d']))
