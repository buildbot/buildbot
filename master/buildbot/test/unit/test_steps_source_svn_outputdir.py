...
class TestSVNOutputDir(sourcesteps.SourceStepMixin, unittest.TestCase):
    ...
    def test_mode_incremental(self):
        self.setupStep(
                svn.SVN(svnurl='http://svn.local/app/trunk@HEAD',
                        ...
                        password='pass', extra_args=['--random'],outputdir='trunk'))
        self.expectCommands(
            ExpectShell(workdir='wkdir',
                        command=['svn', '--version'])
            + 0,
            ExpectLogged('stat', dict(file='wkdir/trunk/.svn',
                                      logEnviron=True))
            + 0,
            ExpectShell(workdir='wkdir',
                        command=['svn', 'update', 'trunk', '--non-interactive',
                                 '--no-auth-cache', '--username', 'user',
                                 '--password', 'pass', '--random'])
            + 0,
            ExpectShell(workdir='wkdir',
                        command=['svnversion', 'trunk'])
    ...
    def test_mode_incremental_baseURL(self):
        self.setupStep(
                svn.SVN(baseURL='http://svn.local/', mode='incremental',
                        defaultBranch='trunk',outputdir='trunk'))
        self.expectCommands(
            ExpectShell(workdir='wkdir',
                        command=['svn', '--version'])
            + 0,
    ...
    def test_mode_incremental_baseURL_not_updatable(self):
        ...
        self.expectCommands(
            ...
            + 1,
            ExpectShell(workdir='wkdir',
                        command=['svn', 'checkout', 'http://svn.local/trunk/app', 'trunk',
    ...
    def test_mode_incremental_given_revision(self):
        self.setupStep(
                svn.SVN(svnurl='http://svn.local/app/trunk@HEAD',
                                    mode='incremental',outputdir='trunk'), dict(
                revision='100',
                ))
        self.expectCommands(
            ExpectShell(workdir='wkdir',
            ...
            + 0,
            ExpectShell(workdir='wkdir',
                        command=['svn', 'update', 'trunk', '--revision', '100',
                                 '--non-interactive', '--no-auth-cache'])
            + 0,
            ExpectShell(workdir='wkdir',
                        command=['svnversion', 'trunk'])
    ...
    def test_mode_full_clobber(self):
        self.setupStep(
                svn.SVN(svnurl='http://svn.local/app/trunk@HEAD',
                                    mode='full', method='clobber',outputdir='trunk'))
        self.expectCommands(
            ExpectShell(workdir='wkdir',
                        command=['svn', '--version'])
            + 0,
            ExpectLogged('rmdir', {'dir': 'wkdir/trunk',
                                   'logEnviron': True})
            + 0,
            ExpectShell(workdir='wkdir',
                        command=['svn', 'checkout',
                                 'http://svn.local/app/trunk@HEAD', 'trunk',
                                 '--non-interactive', '--no-auth-cache'])
            + 0,
            ExpectShell(workdir='wkdir',
                        command=['svnversion', 'trunk'])
    ...
    def test_mode_full_fresh(self):
        self.setupStep(
                svn.SVN(svnurl='http://svn.local/app/trunk@HEAD',
                                    mode='full', method='fresh', depth='infinite', outputdir='trunk'))
        self.expectCommands(
            ExpectShell(workdir='wkdir',
                        command=['svn', '--version'])
            + 0,
            ExpectLogged('stat', {'file': 'wkdir/trunk/.svn',
                                  'logEnviron': True})
            + 0,
            ExpectShell(workdir='wkdir',
                        command=['svn',
                                 'status', '--xml', '--no-ignore', 'trunk',
                                 '--non-interactive', '--no-auth-cache',
                                 '--depth', 'infinite'])
            + ExpectShell.log('stdio',
    ...
    def test_mode_full_fresh_keep_on_purge(self):
        self.setupStep(
                svn.SVN(svnurl='http://svn.local/app/trunk@HEAD',
                        mode='full',
                        keep_on_purge=['svn_external_path/unversioned_file1'],outputdir='trunk'))
        ...
        self.expectCommands(
            ...
            ExpectLogged('rmdir', {'dir':
                                   'wkdir/trunk/svn_external_path/unversioned_file2',
                                   'logEnviron': True})
            + 0,
            ExpectShell(workdir='wkdir',
                        command=['svn', 'update', 'trunk',
    ...
    def test_mode_full_clean(self):
        self.setupStep(
                svn.SVN(svnurl='http://svn.local/app/trunk@HEAD',
                                    mode='full', method='clean', outputdir='trunk'))
        self.expectCommands(
            ExpectShell(workdir='wkdir',
                        command=['svn', '--version'])
            + 0,
        ...
        self.expectCommands(
            ...
            ExpectShell(workdir='wkdir',
                        command=['svn', 'checkout', 'http://svn.local/app/trunk@HEAD',
                                 'trunk', '--non-interactive', '--no-auth-cache'])
            + 0,
            ExpectShell(workdir='wkdir',
                        command=['svnversion', 'trunk'])
            + ExpectShell.log('stdio', stdout='100')
