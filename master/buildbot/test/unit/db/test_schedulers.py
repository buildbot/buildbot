...
class Tests(interfaces.InterfaceTests):
    ...
    def test_enable(self):
        yield self.insertTestData([self.scheduler24, self.master13, self.scheduler24master])
        sch = yield self.db.schedulers.getScheduler(24)
        validation.verifyDbDict(self, "schedulerdict", sch)
        self.assertEqual(sch, dict(id=24, name="schname", enabled=True, masterid=13))

        yield self.db.schedulers.enable(24, False)
        sch = yield self.db.schedulers.getScheduler(24)
        validation.verifyDbDict(self, "schedulerdict", sch)
        self.assertEqual(sch, dict(id=24, name="schname", enabled=False, masterid=13))
    ...
    def test_classifyChanges(self):
        yield self.insertTestData([self.ss92, self.change3, self.change4, self.scheduler24])
    ...
    def test_flushChangeClassifications(self):
        yield self.insertTestData(
            [self.ss92, self.change3, self.change4, self.change5, self.scheduler24]
        )
    ...
    def test_getScheduler(self):
        ...
        sch = yield self.db.schedulers.getScheduler(24)
        validation.verifyDbDict(self, "schedulerdict", sch)
        self.assertEqual(sch, dict(id=24, name="schname", enabled=True, masterid=None))
    ...
    def test_getScheduler_inactive_but_linked(self):
        yield self.insertTestData([self.scheduler25, self.master14, self.scheduler25master])
        sch = yield self.db.schedulers.getScheduler(25)
        validation.verifyDbDict(self, "schedulerdict", sch)
        self.assertEqual(
            sch, dict(id=25, name="schname2", enabled=True, masterid=14)
    ...
    def test_getSchedulers_active(self):
        yield self.insertTestData(
            [self.scheduler24, self.master13, self.scheduler24master, self.scheduler25]
        )
