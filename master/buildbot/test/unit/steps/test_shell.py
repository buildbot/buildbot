...
class TestShellCommandExecution(
    ...
    def test_run_env(self):
        ...
        self.expect_commands(
            ExpectShell(workdir="build", command="echo hello", env=dict(DEF="HERE")).exit(0)
        )
    ...
    def test_run_env_override(self):
        ...
        self.expect_commands(
            ExpectShell(
                workdir="build", command="echo hello", env=dict(ABC="123", DEF="HERE")
            ).exit(0)
        )
        self.expect_outcome(result=SUCCESS)
        return self.run_step()
...
class WarningCountingShellCommand(
    ...
    def test_warn_with_decoderc(self):
        self.setup_step(
            shell.WarningCountingShellCommand(command=["make"], decodeRC={3: WARNINGS})
        )
        self.expect_commands(
            ExpectShell(
                workdir="wkdir",
