...
class TestSVN(sourcesteps.SourceStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_mode_incremental_repourl_not_updatable_svninfo_mismatch(self):
        ...
        self.expect_commands(
            ...
            # expecting ../trunk/app
            .stdout('<?xml version="1.0"?><url>http://svn.local/branch/foo/app</url>').exit(0),
            ExpectRmdir(dir="wkdir", log_environ=True, timeout=1200).exit(0),
            ExpectShell(
                workdir="wkdir",
                command=[
    ...
    def test_mode_full_clean_old_rmdir(self):
        ...
        self.expect_commands(
            ...
            .exit(0),
            ExpectRmdir(
                dir="wkdir/svn_external_path/unversioned_file1",
                log_environ=True,
                timeout=1200,
            ).exit(0),
            ExpectRmdir(
    ...
    def test_mode_full_export_given_revision(self):
        ...
        self.expect_commands(
            ...
            ExpectShell(
                workdir="",
                command=["svn", "export", "--revision", "100", "source", "wkdir"],
            ).exit(0),
            ExpectShell(workdir="source", command=["svn", "info", "--xml"])
            .stdout(self.svn_info_stdout_xml)
            .exit(0),
    ...
    def test_worker_connection_lost(self):
        ...
        self.expect_commands(
            ExpectShell(workdir="wkdir", command=["svn", "--version"]).error(
...
class TestSvnUriCanonicalize(unittest.TestCase):
    @parameterized.expand(
        [
            ...
            (
                "lc_hostname_with_user",
                "http://Jimmy@fOO.Com/bar",
                "http://Jimmy@foo.com/bar",
            ),
            (
            ...
            (
                "dont_remove_port_80",
                "https://foo.com:80/bar",
                "https://foo.com:80/bar",
            ),  # not http
            ("remove_port_443", "https://foo.com:443/bar", "https://foo.com/bar"),
            (
                "dont_remove_port_443",
                "svn://foo.com:443/bar",
                "svn://foo.com:443/bar",
            ),  # not https
            ("remove_port_3690", "svn://foo.com:3690/bar", "svn://foo.com/bar"),
            (
                "dont_remove_port_3690",
                "http://foo.com:3690/bar",
                "http://foo.com:3690/bar",
            ),  # not svn
            (
                "dont_remove_port_other",
                "https://foo.com:2093/bar",
                "https://foo.com:2093/bar",
            ),
            (
                "quote_funny_chars",
                "http://foo.com/\x10\xe6%",
                "http://foo.com/%10%E6%25",
            ),
            (
                "overquoted",
