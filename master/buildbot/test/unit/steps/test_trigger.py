...
class TestTrigger(TestBuildStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_unimportantSchedulerNames_not_in_schedulerNames_but_rendered(self):
        # should not raise
        trigger.Trigger(
            schedulerNames=[properties.Interpolate("a")],
            unimportantSchedulerNames=["b"],
        )

    def test_sourceStamp_and_updateSourceStamp(self):
        with self.assertRaises(config.ConfigErrors):
            trigger.Trigger(schedulerNames=["c"], sourceStamp=dict(x=1), updateSourceStamp=True)
    ...
    def test_sourceStamp_and_alwaysUseLatest(self):
        with self.assertRaises(config.ConfigErrors):
            trigger.Trigger(schedulerNames=["c"], sourceStamp=dict(x=1), alwaysUseLatest=True)
    ...
    def test_set_properties(self):
        yield self.setup_step(trigger.Trigger(schedulerNames=["a"], set_properties=dict(x=1, y=2)))
        self.expect_outcome(result=SUCCESS)
        self.expectTriggeredWith(a=(False, [], dict(x=(1, "Trigger"), y=(2, "Trigger"))))
        yield self.run_step()
    ...
    def test_set_properties_prop(self):
        ...
        self.expect_outcome(result=SUCCESS)
        self.expectTriggeredWith(a=(False, [], dict(x=("xxx", "Trigger"), y=(2, "Trigger"))))
        yield self.run_step()
    ...
    def test_copy_properties(self):
        ...
        self.expect_outcome(result=SUCCESS)
        self.expectTriggeredWith(a=(False, [], dict(a=("A", "Trigger"), b=("B", "Trigger"))))
        yield self.run_step()
