...
class TestGit(
    ...
    def test_getDescription_dirty_none(self):
        self.setup_getDescription_test(
            setup_args={"dirty": None}, output_args=[], expect_head=True
        )
        return self.run_step()
    ...
    def test_getDescription_candidates(self):
        self.setup_getDescription_test(
            setup_args={"candidates": 7}, output_args=["--candidates=7"]
        )
        return self.run_step()

    def test_getDescription_candidates_zero(self):
        self.setup_getDescription_test(
            setup_args={"candidates": 0}, output_args=["--candidates=0"]
        )
        return self.run_step()
    ...
    def test_config_option(self):
        ...
        self.expect_commands(
            ...
            ExpectShell(workdir="wkdir", command=prefix + ["checkout", "-f", "FETCH_HEAD"]).exit(
                0
            ),
