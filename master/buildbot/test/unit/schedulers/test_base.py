...
class BaseScheduler(scheduler.SchedulerMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_change_consumption_fileIsImportant_True(self):
        return self.do_test_change_consumption(dict(fileIsImportant=lambda c: True), True)

    def test_change_consumption_fileIsImportant_False(self):
        return self.do_test_change_consumption(dict(fileIsImportant=lambda c: False), False)

    @defer.inlineCallbacks
    def test_change_consumption_fileIsImportant_exception(self):
        yield self.do_test_change_consumption(dict(fileIsImportant=lambda c: 1 / 0), None)
    ...
    def test_change_consumption_change_filter_True(self):
        ...
        return self.do_test_change_consumption(dict(change_filter=cf), True)

    def test_change_consumption_change_filter_False(self):
        cf = mock.Mock()
        cf.filter_change = lambda c: False
        return self.do_test_change_consumption(dict(change_filter=cf), None)
    ...
    def test_change_consumption_change_filter_gerrit_filters_branch_re_deprecated_no_match(
        self,
    ):
        with assertProducesWarning(
            DeprecatedApiWarning, "Change filters must not expect ref-updated events"
    ...
    def test_change_consumption_fileIsImportant_False_onlyImportant(self):
        return self.do_test_change_consumption(
            dict(fileIsImportant=lambda c: False, onlyImportant=True), None
        )

    def test_change_consumption_fileIsImportant_True_onlyImportant(self):
        return self.do_test_change_consumption(
            dict(fileIsImportant=lambda c: True, onlyImportant=True), True
        )
    ...
    def test_addBuildsetForSourceStampsWithDefaults(self):
        codebases = {
            "cbA": dict(repository="svn://A..", branch="stable", revision="13579"),
            "cbB": dict(repository="svn://B..", branch="stable", revision="24680"),
        }
        sourcestamps = [
    ...
    def test_addBuildsetForChanges_multiple_changes_same_codebase(self):
        ...
        self.master.data.updates.addBuildset.assert_called_with(
            ...
            scheduler="n",
            sourcestamps=[10],
        )  # sourcestampid from greatest changeid
    ...
    def test_addBuildsetForChanges_codebases_set_multiple_codebases(self):
        codebases = {
            "cbA": dict(repository="svn://A..", branch="stable", revision="13579"),
            "cbB": dict(repository="svn://B..", branch="stable", revision="24680"),
            "cbC": dict(repository="svn://C..", branch="stable", revision="12345"),
