...
class Nightly(scheduler.SchedulerMixin, TestReactorMixin, unittest.TestCase):
    ...
    try:
        ...
        skip = "Python 3.6 bug on Windows: " "https://bugs.python.org/issue29097"
    ...
    def test_iterations_simple(self):
        ...
        self.db.state.assertStateByClass(
            "test", "Nightly", last_build=1260 + self.localtime_offset
        )
    ...
    def test_iterations_simple_with_branch(self):
        ...
        self.db.state.assertStateByClass("test", "Nightly", last_build=300 + self.localtime_offset)

        d = sched.deactivate()
        return d

    def do_test_iterations_onlyIfChanged(self, *changes_at, last_only_if_changed=None, **kwargs):
    ...
    def do_test_iterations_onlyIfChanged_test(self, fII, *changes_at):
        ...
        while self.reactor.seconds() < self.localtime_offset + 30 * 60:
            ...
            while (
                changes_at and self.reactor.seconds() >= self.localtime_offset + changes_at[0][0]
            ):
    ...
    def test_iterations_onlyIfChanged_no_changes_new_scheduler(self):
        ...
        self.db.state.assertStateByClass(
            "test", "Nightly", last_build=1500 + self.localtime_offset
        )
    ...
    def test_iterations_onlyIfChanged_unimp_changes_calls_for_new_scheduler(self):
        yield self.do_test_iterations_onlyIfChanged(
            (60, mock.Mock(), False), (600, mock.Mock(), False)
        )
        self.assertEqual(
            self.addBuildsetCalls,
            [
    ...
    @defer.inlineCallbacks
    def test_iterations_onlyIfChanged_unimp_changes_existing_scheduler_setting_changed(
        self,
    ):
        yield self.do_test_iterations_onlyIfChanged(
            (60, mock.Mock(), False),
