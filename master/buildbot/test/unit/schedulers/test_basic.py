...
class CommonStuffMixin:
    def makeScheduler(self, klass, **kwargs_override):
        kwargs = dict(name="tsched", treeStableTimer=60, builderNames=["tbuild"])
...
class SingleBranchScheduler(
    ...
    def test_constructor_no_reason(self):
        ...
        self.assertEqual(
            sched.reason,
            "The SingleBranchScheduler scheduler named 'tsched' triggered this build",
        )
    ...
    def test_gotChange_createAbsoluteSourceStamps_older_change(self):
        ...
        # change 20
        yield sched.gotChange(
            self.mkch(codebase="a", revision="1234:abc", repository="A", number=10),
            True,
        )
    ...
    def test_getCodebaseDict(self):
        ...
        self.assertEqual(
            cbd,
            dict(branch="master", repository="A", revision="5555:def", lastChange=20),
        )
