...
class TestBuildStep(
    ...
    class LockBuildStep(buildstep.BuildStep):
        ...
        def run(self):
            ...
            real_master_lock = yield botmaster.getLockFromLockAccess(
                self.lock_accesses[0], self.build.config_version
            )
            real_worker_lock = yield botmaster.getLockFromLockAccess(
                self.lock_accesses[1], self.build.config_version
            )

            self.testcase.assertFalse(
                real_master_lock.isAvailable(self.testcase, self.lock_accesses[0])
            )
    ...
    def test_renderableLocks(self):
        ...
        real_master_lock = yield botmaster.getLockFromLockAccess(
            lock_accesses[0], self.build.config_version
        )
        real_worker_lock = yield botmaster.getLockFromLockAccess(
            lock_accesses[1], self.build.config_version
        )
    ...
    def test_cancelWhileLocksAvailable(self):
        ...
        stepa = self.setup_step(
            self.FakeBuildStep(locks=[(real_lock1, locks.LockAccess(lock1, "exclusive"))])
        )
        stepb = self.setup_step(
            self.FakeBuildStep(locks=[(real_lock2, locks.LockAccess(lock2, "exclusive"))])
        )
