...
class TestGitPoller(gpo.GetProcessOutputMixin,
    ...
    def setUp(self):
        ...
        def create_poller(_):
            ...
            self.poller.master.db.users.checkFromGit = self.checkFromGit
    ...
    def test_poll(self):
        ...
        def check_changes(_):
            ...
            self.assertEqual(self.changes_added[0]['uid'], 1)
            ...
            self.assertEqual(self.changes_added[1]['uid'], 2)
        d.addCallback(check_changes)

        def check_users(_):
            self.assertEqual(len(self.stored_users), 2)
            self.assertEqual(self.stored_users[0]['id'], 1)
            self.assertEqual(self.stored_users[0]['uid'], 1)
            self.assertEqual(self.stored_users[0]['identifier'], 'by:4423cdbc')
            self.assertEqual(self.stored_users[0]['auth_type'], 'full_name')
            self.assertEqual(self.stored_users[0]['auth_data'], 'by:4423cdbc')

            self.assertEqual(self.stored_users[1]['id'], 2)
            self.assertEqual(self.stored_users[1]['uid'], 2)
            self.assertEqual(self.stored_users[1]['identifier'], 'by:64a5dc2a')
            self.assertEqual(self.stored_users[1]['auth_type'], 'full_name')
            self.assertEqual(self.stored_users[1]['auth_data'], 'by:64a5dc2a')
