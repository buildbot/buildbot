...
class HTTPClientServiceTestBase(unittest.TestCase):
    ...
    def setUp(self) -> InlineCallbacksType[None]:  # type: ignore[override]
        ...
        self.parent.reactor = reactor  # type: ignore[attr-defined]
...
class HTTPClientServiceTestTxRequest(HTTPClientServiceTestBase):
    ...
    @defer.inlineCallbacks
    def test_get_full_url(self) -> InlineCallbacksType[None]:
        with assertProducesWarning(DeprecationWarning):
    ...
    @defer.inlineCallbacks
    def test_post(self) -> InlineCallbacksType[None]:
        with assertProducesWarning(DeprecationWarning):
    ...
    @defer.inlineCallbacks
    def test_delete(self) -> InlineCallbacksType[None]:
        with assertProducesWarning(DeprecationWarning):
    ...
    @defer.inlineCallbacks
    def test_post_headers(self) -> InlineCallbacksType[None]:
    ...
    @defer.inlineCallbacks
    def test_post_auth(self) -> InlineCallbacksType[None]:
        self._http = yield httpclientservice.HTTPClientService.getService(
    ...
    @defer.inlineCallbacks
    def test_post_mtls(self) -> InlineCallbacksType[None]:
        self._http = yield httpclientservice.HTTPClientService.getService(
            self.parent,
            'http://foo',
...
class HTTPClientServiceTestTxRequestNoEncoding(HTTPClientServiceTestBase):
    ...
    def setUp(self) -> InlineCallbacksType[None]:  # type: ignore[override]
        yield super().setUp()
        self._http = self.successResultOf(  # type: ignore[assignment]
    ...
    @defer.inlineCallbacks
    def test_post_raw(self) -> InlineCallbacksType[None]:
        with assertProducesWarning(DeprecationWarning):
            yield self._http.post('/bar', json={'foo': 'bar'})
        jsonStr = json.dumps({"foo": 'bar'})
        headers = {'Content-Type': 'application/json'}
    ...
    @defer.inlineCallbacks
    def test_post_rawlist(self) -> InlineCallbacksType[None]:
        with assertProducesWarning(DeprecationWarning):
            yield self._http.post('/bar', json=[{'foo': 'bar'}])
        jsonStr = json.dumps([{"foo": 'bar'}])
        headers = {'Content-Type': 'application/json'}
...
class HTTPClientServiceTestTReq(HTTPClientServiceTestBase):
    ...
    def test_get(self) -> InlineCallbacksType[None]:
        with assertProducesWarning(DeprecationWarning):
            yield self._http.get('/bar')
        httpclientservice.treq.get.assert_called_once_with(  # type: ignore[attr-defined]
    ...
    def test_put(self) -> InlineCallbacksType[None]:
        with assertProducesWarning(DeprecationWarning):
            yield self._http.put('/bar', json={'foo': 'bar'})
        headers = {'Content-Type': ['application/json']}
        httpclientservice.treq.put.assert_called_once_with(  # type: ignore[attr-defined]
    ...
    def test_post(self) -> InlineCallbacksType[None]:
        with assertProducesWarning(DeprecationWarning):
            yield self._http.post('/bar', json={'foo': 'bar'})
        headers = {'Content-Type': ['application/json']}
        httpclientservice.treq.post.assert_called_once_with(  # type: ignore[attr-defined]
            'http://foo/bar', agent=mock.ANY, data=b'{"foo": "bar"}', headers=headers
        )
    ...
    def test_delete(self) -> InlineCallbacksType[None]:
        with assertProducesWarning(DeprecationWarning):
            yield self._http.delete('/bar')
        httpclientservice.treq.delete.assert_called_once_with(  # type: ignore[attr-defined]
            'http://foo/bar', agent=mock.ANY, headers={}
        )
    ...
    @defer.inlineCallbacks
    def test_post_auth_digest(self) -> InlineCallbacksType[None]:
...
class MyResource(resource.Resource):
    ...
    def render_GET(self, request: Request) -> bytes:
        ...
        if content_type == b"application/json":
            jsonBytes = request.content.read()  # type: ignore[attr-defined]
            jsonStr = bytes2unicode(jsonBytes)
            args['json_received'] = json.loads(jsonStr)

        data = json.dumps(args)
        data_bytes = unicode2bytes(data)
        request.setHeader(b'content-type', b'application/json')
        request.setHeader(b'content-length', b"%d" % len(data_bytes))
...
class HTTPClientServiceTestTxRequestE2E(unittest.TestCase):
    ...
    def httpFactory(
        self, parent: service.MasterService
    ) -> defer.Deferred[httpclientservice.HTTPClientService]:
        return httpclientservice.HTTPClientService.getService(  # type: ignore[return-value]
            parent, f'http://127.0.0.1:{self.port}'
        )

    def expect(self, *arg: Any, **kwargs: Any) -> None:
        pass
    ...
    def setUp(self) -> InlineCallbacksType[None]:  # type: ignore[override]
        ...
        self.listenport = reactor.listenTCP(0, self.site)  # type: ignore[attr-defined]
    ...
    @defer.inlineCallbacks
    def test_content(self) -> InlineCallbacksType[None]:
    ...
    @defer.inlineCallbacks
    def test_content_with_params(self) -> InlineCallbacksType[None]:
    ...
    @defer.inlineCallbacks
    def test_post_content_with_params(self) -> InlineCallbacksType[None]:
    ...
    @defer.inlineCallbacks
    def test_put_content_with_data(self) -> InlineCallbacksType[None]:
    ...
    @defer.inlineCallbacks
    def test_put_content_with_json(self) -> InlineCallbacksType[None]:
        ...
        content = json.loads(content)  # type: ignore[arg-type]
        self.assertEqual(content, exp_content_json)

    @defer.inlineCallbacks
    def test_put_content_with_json_datetime(self) -> InlineCallbacksType[None]:
    ...
    @defer.inlineCallbacks
    def test_json(self) -> InlineCallbacksType[None]:
        self.expect('get', '/', content_json={})
        with assertProducesWarning(DeprecationWarning):
            res = yield self._http.get('/')
    ...
    @defer.inlineCallbacks
    def test_lots(self) -> InlineCallbacksType[None]:
    ...
    @defer.inlineCallbacks
    def test_lots_parallel(self) -> InlineCallbacksType[None]:
        for _ in range(self.NUM_PARALLEL):
            self.expect('get', '/', params={"a": 'b'}, content_json={"a": ['b']})

        # use for benchmarking (txrequests: 3ms per request treq: 11ms per
        # request (!?))
        def oneReq() -> defer.Deferred[bytes]:
            with assertProducesWarning(DeprecationWarning):
                d = self._http.get('/', params={"a": 'b'})

            @d.addCallback
            def content(res: interfaces.IHttpResponse) -> defer.Deferred[bytes]:
                return res.content()
...
class HTTPClientServiceTestFakeE2E(HTTPClientServiceTestTxRequestE2E):
    ...
    def httpFactory(
        self, parent: service.MasterService
    ) -> InlineCallbacksType[httpclientservice.HTTPClientService]:
        svc = yield fakehttpclientservice.HTTPClientService.getService(
