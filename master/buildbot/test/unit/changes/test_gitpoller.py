...
class TestGitPollerBareRepository(
    ...
    MERGE_FEATURE_1_SHA = "756ded5206ccd71723895997d82382ed33ddf9d4"
    ...
    async def prepare_repository(self):
        ...
        initial_commit_hash = self.repo.commit(
            ...
            message="Initial",
            date=_date(datetime.timedelta(minutes=1)),
        ...
        fix_1_hash = self.repo.commit(
            ...
            author_mail=author_mail,
            message="Fix 1",
            date=_date(datetime.timedelta(minutes=2)),
            files=[readme_path.relative_to(self.repo.repository_path)],
        )
        ...
        feature_1_hash = self.repo.commit(
            ...
            author_mail=author_mail,
            message="Feature 1",
            date=_date(datetime.timedelta(minutes=3)),
            files=[readme_path.relative_to(self.repo.repository_path)],
        )
        ...
        try:
            self.repo.exec_git(['merge', '--no-ff', '--no-commit', '--strategy=ours', 'feature/1'])
        ...
        with readme_path.open('a') as fp:
            fp.write("initial\n\nfix 1\nfeature 1\n")
        _set_utime(_date(datetime.timedelta(minutes=4)))
        ...
        merge_feature_1_hash = self.repo.commit(
            ...
            date=_date(datetime.timedelta(minutes=4)),
        )
