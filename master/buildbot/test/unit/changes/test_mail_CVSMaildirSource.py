...
class TestCVSMaildirSource(unittest.TestCase):
    def test_CVSMaildirSource_create_change_from_cvs1_11msg(self) -> None:
        m = message_from_string(cvs1_11_msg)
        src = CVSMaildirSource('/dev/null')
        src, chdict = src.parse(m)  # type: ignore[assignment, misc]
        ...
        self.assertEqual(chdict['when'], mktime_tz(dateTuple))  # type: ignore[arg-type]
    ...
    def test_CVSMaildirSource_create_change_from_cvs1_12msg(self) -> None:
    ...
    def test_CVSMaildirSource_create_change_from_cvs1_12_with_no_path(self) -> None:
        msg = cvs1_12_msg.replace('Path: base/module/src', '')
        m = message_from_string(msg)
        src = CVSMaildirSource('/dev/null')
        try:
            assert src.parse(m)[1]  # type: ignore[index]
        except ValueError:
            pass
        else:
            self.fail('Expect ValueError.')

    def test_CVSMaildirSource_create_change_with_bad_cvsmode(self) -> None:
    ...
    def test_CVSMaildirSource_create_change_with_branch(self) -> None:
        # Branch is indicated after 'Tag:' in modified file list
        msg = cvs1_11_msg.replace(
            '        GNUmakefile', '      Tag: Test_Branch\n      GNUmakefile'
        )
        m = message_from_string(msg)
        src = CVSMaildirSource('/dev/null')
        chdict = src.parse(m)[1]  # type: ignore[index]
        self.assertEqual(chdict['branch'], 'Test_Branch')

    def test_CVSMaildirSource_create_change_with_category(self) -> None:
    ...
    def test_CVSMaildirSource_create_change_with_no_comment(self) -> None:
        # Strip off comments
    ...
    def test_CVSMaildirSource_create_change_with_no_files(self) -> None:
    ...
    def test_CVSMaildirSource_create_change_with_no_project(self) -> None:
    ...
    def test_CVSMaildirSource_create_change_with_no_repository(self) -> None:
    ...
    def test_CVSMaildirSource_create_change_with_property(self) -> None:
        m = message_from_string(cvs1_11_msg)
        propDict = {'foo': 'bar'}
