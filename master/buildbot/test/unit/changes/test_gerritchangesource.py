...
class TestGerritHelpers(unittest.TestCase):
    ...
    def test_missing_username(self):
        self.assertEqual(
            "Justin Case <justin.case@example.com>",
            gerritchangesource._gerrit_user_to_author(
                {"name": "Justin Case", "email": "justin.case@example.com"}
            ),
        )
    ...
    def test_missing_email(self):
        self.assertEqual(
            "Justin Case",
            gerritchangesource._gerrit_user_to_author(
                {"username": "justincase", "name": "Justin Case"}
            ),
        )
        self.assertEqual(
            "Justin Case",
        ...
        self.assertEqual(
            "justincase",
            gerritchangesource._gerrit_user_to_author({"username": "justincase"}),
        )
...
class TestGerritChangeSource(
    ...
    query_files_success = "\n".join(
        [json.dumps(query_files_success_line1), json.dumps(query_files_success_line2)]
    ).encode("utf8")
...
class TestGerritEventLogPoller(
    ...
    def test_lineReceived_patchset_created(self):
        ...
        thirty_days_ago = datetime.datetime.utcfromtimestamp(
            self.NOW_TIMESTAMP
        ...
        self._http.expect(
            ...
            content_json=dict(
                ...
                change=dict(
                    ...
                    project="test",
                    number="4321",
                    owner=dict(name="owner owner", email="owner@example.com"),
                    url="http://example.com/c/test/+/4321",
                    subject="change subject",
                ),
