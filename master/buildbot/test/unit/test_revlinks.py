...
class TestGithubRevlink(unittest.TestCase):
    ...
    def testHTTPS(self):
        self.assertEqual(
            GithubRevlink(self.revision, "https://github.com/buildbot/buildbot.git"),
            self.url,
        )

    def testGIT(self):
        self.assertEqual(
            GithubRevlink(self.revision, "git://github.com/buildbot/buildbot.git"),
            self.url,
        )
    ...
    def testSSHuri(self):
        self.assertEqual(
            GithubRevlink(self.revision, "ssh://git@github.com/buildbot/buildbot.git"),
            self.url,
        )
...
class TestSourceforgeGitRevlink_AlluraPlatform(unittest.TestCase):
    ...
    url = (
        "https://sourceforge.net/p/klusters/klusters/ci/6f9b1470bae497c6ce47e4cf8c9195d864d2ba2f/"
    )
...
class TestBitbucketRevlink(unittest.TestCase):
    ...
    url = (
        "https://bitbucket.org/fakeproj/fakerepo/commits/4d4284cf4fb49ce82fefb6cbac8e462073c5f106"
    )

    def testHTTPS(self):
        self.assertEqual(
            BitbucketRevlink(
                self.revision, "https://fakeuser@bitbucket.org/fakeproj/fakerepo.git"
            ),
            self.url,
        )
    ...
    def testSSH(self):
        self.assertEqual(
            BitbucketRevlink(self.revision, "git@bitbucket.org:fakeproj/fakerepo.git"),
            self.url,
        )
