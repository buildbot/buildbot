...
class Migration(migration.MigrateTestMixin, unittest.TestCase):
    ...
    def test_empty_migration(self):
        ...
        def verify_thd(conn):
            ...
            buildworkers = sa.Table('buildworkers', metadata, autoload=True)

            # table starts empty
            res = conn.execute(buildworkers.select())
            self.assertEqual(res.fetchall(), [])

            # and buildworker name is unique, so we'll get an error here
