    ...
    def test_addSourceStamp_patch(self):
        ...
        def check(ssid):
            def thd(conn):
                ...
                rows = [ (row.id, row.patchlevel, row.patch_base64, row.subdir, row.patch_author)
                         for row in r.fetchall() ]
                self.assertEqual(rows, [(patchid, 3, 'bXkgcGF0Y2g=', 'master/', 'me')])
            return self.db.pool.do(thd)
        d.addCallback(check)
        return d
    ...
    def test_getSourceStamp_simple(self):
        ...
        def check(ssdict):
            self.assertEqual(ssdict, dict(ssid=234, branch='br', revision='rv',
                ...
                patch_author=None, changeids=set([])))
        d.addCallback(check)
        return d
    ...
    def test_getSourceStamp_patch(self):
        d = self.insertTestData([
            fakedb.Patch(id=99, patch_base64='aGVsbG8sIHdvcmxk',
                patch_author='bar', subdir='/foo', patchlevel=3),
