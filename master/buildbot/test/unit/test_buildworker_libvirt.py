...
from buildbot.buildworker import libvirt as libvirtbuildworker
...
class TestLibVirtWorker(unittest.TestCase):
    ...
    class ConcreteBuildWorker(libvirtbuildworker.LibVirtWorker):
        pass

    def setUp(self):
        self.patch(libvirtbuildworker, "libvirt", libvirt)
        self.conn = libvirtbuildworker.Connection("test://")
        self.lvconn = self.conn.connection

    def test_constructor_nolibvirt(self):
        self.patch(libvirtbuildworker, "libvirt", None)
        self.assertRaises(config.ConfigErrors, self.ConcreteBuildWorker,
                          'bot', 'pass', None, 'path', 'path')

    @defer.inlineCallbacks
    def test_constructor_minimal(self):
        bs = self.ConcreteBuildWorker('bot', 'pass', self.conn, 'path', 'otherpath')
    ...
    def test_find_existing(self):
        ...
        bs = self.ConcreteBuildWorker('bot', 'pass', self.conn, 'p', 'o')
        yield bs._find_existing_deferred
    ...
    def test_prepare_base_image_none(self):
        ...
        bs = self.ConcreteBuildWorker('bot', 'pass', self.conn, 'p', None)
        yield bs._find_existing_deferred
    ...
    def test_start_instance(self):
        bs = self.ConcreteBuildWorker('b', 'p', self.conn, 'p', 'o',
                                     xml='<xml/>')
    ...
    def test_start_instance_create_fails(self):
        ...
        self.patch(libvirtbuildworker.Connection, 'create', create)
...
class TestWorkQueue(unittest.TestCase):
    ...
    def setUp(self):
        self.queue = libvirtbuildworker.WorkQueue()
