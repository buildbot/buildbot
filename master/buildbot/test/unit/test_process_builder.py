...
class BuilderMixin(object):
    ...
    def makeBuilder(self, name="bldr", patch_random=False, noReconfig=False,
        ...
        def _startBuildFor(workerbuilder, buildrequests):
            self.builds_started.append((workerbuilder, buildrequests))
...
class TestBuilder(BuilderMixin, unittest.TestCase):
    ...
    def setWorkerBuilders(self, workerbuilders):
    ...
    def test_enforceChosenWorker(self):
        ...
        worker = mock.Mock()
        worker.worker.workername = 'worker5'

        breq = mock.Mock()

        # no buildworker requested
        breq.properties = {}
        result = yield self.bldr.canStartBuild(worker, breq)
        self.assertIdentical(True, result)

        # buildworker requested as the right one
        breq.properties = {'workername': 'worker5'}
        result = yield self.bldr.canStartBuild(worker, breq)
        self.assertIdentical(True, result)

        # buildworker requested as the wrong one
        breq.properties = {'workername': 'worker4'}
        result = yield self.bldr.canStartBuild(worker, breq)
        self.assertIdentical(False, result)

        # buildworker set to non string value gets skipped
