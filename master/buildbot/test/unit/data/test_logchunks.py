    ...
    def do_test_chunks(self, path, logid, expLines):
        ...
        for i, expLine in enumerate(expLines):
            logchunk = yield self.callGet(
                path, resultSpec=resultspec.ResultSpec(offset=i, limit=1)
            )
            self.validateData(logchunk)
        ...
        # some illegal stuff
        self.assertEqual(
            (yield self.callGet(path, resultSpec=resultspec.ResultSpec(offset=-1))),
            None,
        )
        self.assertEqual(
            (yield self.callGet(path, resultSpec=resultspec.ResultSpec(offset=10, limit=-1))),
            None,
        )
