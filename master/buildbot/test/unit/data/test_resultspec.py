...
class ResultSpecMKListMixin:
    @staticmethod
    def mkdata(fld: Sequence[str] | str, *values: object) -> list[dict[str, object]]:
        if isinstance(fld, str):
            return [{fld: val} for val in values]

        return [dict(zip(fld, cast('Iterable[object]', val))) for val in values]
...
class ResultSpecMKDataclassMixin(ResultSpecMKListMixin):
    ...
    @staticmethod
    def mkdata(fld: Sequence[str] | str, *values: object) -> list[object]:  # type: ignore[override]
        if isinstance(fld, str):
            fields = [fld]
        else:
            fields = sorted(fld)
...
class FilterTestMixin(_Base):
    @staticmethod
    def mkdata(fld: Sequence[str] | str, *values: object) -> list[object]:
    ...
    def test_contains_plural(self) -> None:
...
class FilterList(unittest.TestCase, ResultSpecMKListMixin, FilterTestMixin):  # type: ignore[misc]
    pass


class FilterDataclass(unittest.TestCase, ResultSpecMKDataclassMixin, FilterTestMixin):  # type: ignore[misc]
    pass


class ResultSpecTestMixin(_Base):
    @staticmethod
    def mkdata(fld: Sequence[str] | str, *values: object) -> list[object]:
        raise NotImplementedError()

    def assertListResultEqual(self, a: base.ListResult, b: base.ListResult) -> None:
    ...
    def test_apply_details_fields(self) -> None:
    ...
    def test_apply_collection_fields(self) -> None:
    ...
    def test_apply_missing_fields(self) -> None:
        data = self.mkdata(
            ('fn', 'ln'),
            ('cedric', 'willis'),
            ('albert', 'engelbert'),
    ...
    def do_test_pagination(self, bareList: bool) -> None:
        data = self.mkdata('x', *list(range(101, 131)))
        if not bareList:
            data = base.ListResult(data)  # type: ignore[assignment]
            data.offset = None  # type: ignore[attr-defined]
            data.total = len(data)  # type: ignore[attr-defined]
            data.limit = None  # type: ignore[attr-defined]
        self.assertListResultEqual(
    ...
    def test_pagination_bare_list(self) -> None:
        return self.do_test_pagination(bareList=True)

    def test_pagination_ListResult(self) -> None:
        return self.do_test_pagination(bareList=False)

    def test_pagination_prepaginated(self) -> None:
    ...
    def test_pagination_prepaginated_without_clearing_resultspec(self) -> None:
        data = base.ListResult(self.mkdata('x', *list(range(10, 20))))
        data.offset = 10
        data.limit = 10
    ...
    def test_endpoint_returns_total_without_applying_filters(self) -> None:
        data = base.ListResult(self.mkdata('x', *list(range(10, 20))))
        data.total = 99
    ...
    def test_popProperties(self) -> None:
    ...
    def test_popBooleanFilter(self) -> None:
        rs = resultspec.ResultSpec(
            filters=[
    ...
    def test_popStringFilter(self) -> None:
        rs = resultspec.ResultSpec(
            filters=[
    ...
    def test_popStringFilterSeveral(self) -> None:
        rs = resultspec.ResultSpec(
            filters=[
    ...
    def test_popIntegerFilter(self) -> None:
        rs = resultspec.ResultSpec(
            filters=[
    ...
    def test_popIntegerFilterSeveral(self) -> None:
        rs = resultspec.ResultSpec(
            filters=[
    ...
    def test_popIntegerFilterNotInt(self) -> None:
        rs = resultspec.ResultSpec(
            filters=[
    ...
    def test_popField(self) -> None:
        rs = resultspec.ResultSpec(fields=['foo', 'bar'])
        self.assertTrue(rs.popField('foo'))
        self.assertEqual(rs.fields, ['bar'])

    def test_popField_not_present(self) -> None:
        rs = resultspec.ResultSpec(fields=['foo', 'bar'])
        self.assertFalse(rs.popField('nosuch'))
        self.assertEqual(rs.fields, ['foo', 'bar'])


class ResultSpecList(unittest.TestCase, ResultSpecMKListMixin, ResultSpecTestMixin):  # type: ignore[misc]
    def test_apply_missing_fields(self) -> None:
        # note that the REST interface catches this with a nicer error message
        with self.assertRaises(KeyError):  # type: ignore[attr-defined]
            super().test_apply_missing_fields()  # type: ignore[misc]


class ResultSpecDataclass(unittest.TestCase, ResultSpecMKDataclassMixin, ResultSpecTestMixin):  # type: ignore[misc]
    def test_apply_missing_fields(self) -> None:
    ...
    def test_apply_collection_fields(self) -> None:
        with self.assertRaises(TypeError):  # type: ignore[attr-defined]
            super().test_apply_collection_fields()  # type: ignore[misc]


class ComparatorTestMixin(_Base):
    @staticmethod
    def mkdata(fld: Sequence[str] | str, *values: object) -> list[object]:
        raise NotImplementedError()

    def test_noneComparator(self) -> None:
    ...
    def test_noneComparison(self) -> None:
    ...
    def test_reverseComparator(self) -> None:
        reverse35 = ReverseComparator(35)  # type: ignore[arg-type]
        reverse36 = ReverseComparator(36)  # type: ignore[arg-type]
    ...
    def test_reverseComparison(self) -> None:
        nums = [1, 2, 3, 4, 5]
        nums.sort(key=ReverseComparator)  # type: ignore[arg-type]
        self.assertEqual(nums, [5, 4, 3, 2, 1])

    def test_reverseComparisonWithNone(self) -> None:
        noneInList = ["z", None, None, "q", "a", None, "v"]
...
class ComparatorList(unittest.TestCase, ResultSpecMKListMixin, ComparatorTestMixin):  # type: ignore[misc]
    pass


class ComparatorDataclass(unittest.TestCase, ResultSpecMKDataclassMixin, ComparatorTestMixin):  # type: ignore[misc]
    pass
