...
class TestContact(ContactMixin, unittest.TestCase):
    ...
    def test_notify_missing_worker(self):
        ...
        missing_worker(
            (None, None, "missing"),
            dict(workerid=1, name="work", last_connection="sometime"),
        )
    ...
    def test_command_list_workers(self):
        ...
        for worker in workers:
            self.master.db.workers.db.insertTestData([fakedb.Worker(name=worker)])
    ...
    def test_command_list_workers_online(self):
        ...
        self.master.db.insertTestData([fakedb.ConnectedWorker(id=113, masterid=13, workerid=1)])
        yield self.do_test_command("list", args="all workers")
        self.assertEqual(len(self.sent), 1)
    ...
    def test_command_last(self):
        ...
        self.setup_multi_builders()
        # Also set the connectedness:
        self.master.db.insertTestData([fakedb.ConnectedWorker(id=113, masterid=13, workerid=2)])
