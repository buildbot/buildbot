...
class TestGerritStatusPush(TestReactorMixin, unittest.TestCase, ReporterTestMixin):
    ...
    def check_summary_build_deferred(self, buildResults, finalResult, resultText, verifiedScore):
        ...
        gsp.sendCodeReview.assert_called_once_with(
            self.reporter_test_project, self.reporter_test_revision, result
        )
    ...
    def check_summary_build_legacy(self, buildResults, finalResult, resultText, verifiedScore):
        ...
        result = makeReviewResult(
            msg, (GERRIT_LABEL_VERIFIED, verifiedScore), (GERRIT_LABEL_REVIEWED, 0)
        )
        gsp.sendCodeReview.assert_called_once_with(
            self.reporter_test_project, self.reporter_test_revision, result
        )
    ...
    def check_single_build(self, buildResult, verifiedScore):
        ...
        start = makeReviewResult(
            str({"name": self.reporter_test_builder_name}), (GERRIT_LABEL_REVIEWED, 0)
        )
        result = makeReviewResult(msg, (GERRIT_LABEL_VERIFIED, verifiedScore))
        calls = [
            call(self.reporter_test_project, self.reporter_test_revision, start),
    ...
    def testBuildGerritCommand(self):
        ...
        yield gsp.sendCodeReview(
            "project", "revision", {"message": "bla", "labels": {"Verified": 1}}
        )
