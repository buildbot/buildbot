...
class TestIrcStatusBot(unittest.TestCase):
    ...
    def test_register_legacy(self):
        ...
        self.assertEqual(
            events,
            [("l", "PASS pass"), ("l", "NICK bot"), ("l", "USER bot foo bar :None")],
        )
    ...
    def test_format_build_status_colors(self):
        ...
        b.useColors = True
        self.assertEqual(
            b.format_build_status({"results": SUCCESS}),
...
class TestIRC(config.ConfigErrorsMixin, unittest.TestCase):
    ...
    def test_allowForce_allowShutdown(self):
        ...
        yield s.startService()
        self.assertEqual(
            words.StatusBot.expand_authz(s.authz),
            {"FORCE": True, "STOP": True, "SHUTDOWN": False},
        )
