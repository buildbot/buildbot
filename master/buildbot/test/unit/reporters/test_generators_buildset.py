...
class TestBuildSetGeneratorBase(
    ...
    async def setup_generator(
        ...
        if insert_build:
            build = await self.insert_build_finished_get_props(results, **db_args)
            buildset = await self.get_inserted_buildset()
        else:
            build = None
            buildset = await self.insert_buildset_no_builds(results, **db_args)
...
class TestBuildSetGenerator(TestBuildSetGeneratorBase):
    ...
    async def buildset_message(self, g, builds, buildset):
        reporter = Mock()
        reporter.getResponsibleUsersForBuild.return_value = []

        report = await g.buildset_message(g.formatter, self.master, reporter, builds, buildset)
        return report
    ...
    async def test_buildset_message_nominal(self):
        g, build, buildset = await self.setup_generator(mode=("change",))
        report = await self.buildset_message(g, [build], buildset)
    ...
    async def test_buildset_message_no_result(self):
    ...
    async def test_buildset_message_no_result_formatter_no_subject(self):
        message = {
            "body": "body",
            "type": "text",
    ...
    async def test_generate_complete(self):
        g, build, buildset = await self.setup_generator()
        report = await self.generate(g, ('buildsets', 98, 'complete'), buildset)
    ...
    async def test_generate_complete_no_builds(self):
        g, _, buildset = await self.setup_generator(insert_build=False)
        report = await self.generate(g, ('buildsets', 98, 'complete'), buildset)

        g.formatter.format_message_for_build.assert_not_called()
        self.assertIsNone(report)

    async def test_generate_complete_non_matching_builder(self):
        g, _, buildset = await self.setup_generator(builders=['non-matched'])
        report = await self.generate(g, ('buildsets', 98, 'complete'), buildset)

        g.formatter.format_message_for_build.assert_not_called()
        self.assertIsNone(report)

    async def test_generate_complete_non_matching_result(self):
        g, _, buildset = await self.setup_generator(mode=('failing',))
        report = await self.generate(g, ('buildsets', 98, 'complete'), buildset)
...
class TestBuildSetCombinedGenerator(TestBuildSetGeneratorBase):
    ...
    async def buildset_message(self, g, buildset, builds):
        reporter = Mock()
        report = await g.buildset_message(g.formatter, self.master, reporter, buildset, builds)
        return report

    async def generate(self, g, key, buildset):
        report = await g.generate(self.master, Mock(), key, buildset)
        return report

    async def test_buildset_message_normal(self):
        g, build, buildset = await self.setup_generator()
        report = await self.buildset_message(g, buildset, [build])
    ...
    async def test_buildset_message_no_builds(self):
        g, _, buildset = await self.setup_generator(insert_build=False)
        report = await self.buildset_message(g, buildset, [])
    ...
    async def test_buildset_message_no_result(self):
        g, build, buildset = await self.setup_generator(results=None)
        buildset["results"] = None
        report = await self.buildset_message(g, buildset, [build])
    ...
    async def test_buildset_message_no_builds_no_result(self):
        g, _, buildset = await self.setup_generator(results=None, insert_build=False)
        buildset["results"] = None
        report = await self.buildset_message(g, buildset, [])
    ...
    async def test_buildset_message_no_result_formatter_no_subject(self):
        ...
        g, build, buildset = await self.setup_generator(message=message)
        report = await self.buildset_message(g, buildset, [build])
    ...
    async def test_generate_complete_with_builds(self):
        g, build, buildset = await self.setup_generator(insert_build=True)
        report = await self.generate(g, ("buildsets", 98, "complete"), buildset)
