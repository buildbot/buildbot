...
class MasterConfig_loaders(ConfigErrorsMixin, unittest.TestCase):
    ...
    def test_load_global_when_workerPortnum_and_protocols_set(self):
        self.cfg.load_global(self.filename,
                             dict(protocols={"pb": {"port": 123}}, workerPortnum=321))
        self.assertConfigError(self.errors,
                               "Both c['workerPortnum'] and c['protocols']['pb']['port']"
    ...
    def test_load_global_workerPortnum_int(self):
        self.do_test_load_global(dict(workerPortnum=123),
                                 protocols={'pb': {'port': 'tcp:123'}})

    def test_load_global_workerPortnum_str(self):
        self.do_test_load_global(dict(workerPortnum='udp:123'),
    ...
    def test_load_workers_reserved_names(self):
        for name in 'debug', 'change', 'status':
            self.cfg.load_workers(self.filename,
                                 dict(workers=[buildworker.BuildWorker(name, 'x')]))
    ...
    def test_load_workers(self):
        sl = buildworker.BuildWorker('foo', 'x')
        self.cfg.load_workers(self.filename,
                             dict(workers=[sl]))
