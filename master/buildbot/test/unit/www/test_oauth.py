...
class OAuth2Auth(TestReactorMixin, www.WwwTestMixin, ConfigErrorsMixin, unittest.TestCase):
    ...
    def test_GoogleVerifyCode(self):
        ...
        requests.post.side_effect = [FakeResponse(dict(access_token="TOK3N"))]
        self.googleAuth.get = mock.Mock(
            side_effect=[dict(name="foo bar", email="bar@foo", picture="http://pic")]
        )
    ...
    def test_GithubVerifyCode(self):
        ...
        def fake_get(self, ep, **kwargs):
            ...
            if ep == "/user":
                return dict(login="bar", name="foo bar", email="buzz@bar")
            if ep == "/user/emails":
                return [
    ...
    def test_BitbucketVerifyCode(self):
        ...
        self.bitbucketAuth.get = mock.Mock(
            side_effect=[
                dict(username="bar", display_name="foo bar"),  # /user
                dict(  # /user/emails
                    values=[
                ...
                dict(values=[{"slug": "hello"}, {"slug": "grp"}]),  # /workspaces?role=member
            ]
        )
