...
class MasterConfig_loaders(ConfigErrorsMixin, unittest.TestCase):
    ...
    def test_load_global_logCompressionLimit(self):
        self.do_test_load_global(dict(logCompressionLimit=10), logCompressionLimit=10)

    def test_load_global_logCompressionMethod(self):
        self.do_test_load_global(dict(logCompressionMethod="bz2"), logCompressionMethod="bz2")
    ...
    def test_load_global_codebaseGenerator(self):
        func = lambda _: "dummy"
        self.do_test_load_global(dict(codebaseGenerator=func), codebaseGenerator=func)
    ...
    def test_load_global_logEncoding(self):
        self.do_test_load_global(dict(logEncoding="latin-2"), logEncoding="latin-2")
    ...
    def test_load_global_collapseRequests_bool(self):
        self.do_test_load_global(dict(collapseRequests=False), collapseRequests=False)

    def test_load_global_collapseRequests_callable(self):
        callable = lambda: None
        self.do_test_load_global(dict(collapseRequests=callable), collapseRequests=callable)
    ...
    def test_load_global_prioritizeBuilders_callable(self):
        callable = lambda: None
        self.do_test_load_global(dict(prioritizeBuilders=callable), prioritizeBuilders=callable)
    ...
    def test_load_global_protocols_str(self):
        self.do_test_load_global(
            dict(protocols={"pb": {"port": "udp:123"}}),
    ...
    def test_load_global_revlink_callable(self):
        callable = lambda: None
        self.do_test_load_global(dict(revlink=callable), revlink=callable)
    ...
    def test_load_validation(self):
        r = re.compile(".*")
        self.cfg.load_validation(self.filename, dict(validation=dict(branch=r)))
    ...
    def test_load_mq_explicit_type(self):
        self.cfg.load_mq(self.filename, dict(mq=dict(type="simple")))
        self.assertResults(mq=dict(type="simple"))
    ...
    def test_load_metrics(self):
        self.cfg.load_metrics(self.filename, dict(metrics=dict(foo=1)))
    ...
    def test_load_caches_buildCacheSize(self):
        self.cfg.load_caches(self.filename, dict(buildCacheSize=13))
    ...
    def test_load_caches_changeCacheSize(self):
        self.cfg.load_caches(self.filename, dict(changeCacheSize=13))
    ...
    def test_load_caches(self):
        self.cfg.load_caches(self.filename, dict(caches=dict(foo=1)))
    ...
    def test_load_schedulers(self):
        ...
        self.cfg.load_schedulers(self.filename, dict(schedulers=[sch]))
    ...
    def test_load_workers(self):
        ...
        self.cfg.load_workers(self.filename, dict(workers=[wrk]))
    ...
    def test_load_change_sources_single(self):
        ...
        self.cfg.load_change_sources(self.filename, dict(change_source=chsrc))
    ...
    def test_load_machines_list(self):
        mm = FakeMachine()
        self.cfg.load_machines(self.filename, dict(machines=[mm]))
    ...
    def test_load_user_managers(self):
        um = mock.Mock()
        self.cfg.load_user_managers(self.filename, dict(user_managers=[um]))
    ...
    def test_load_www_port(self):
        self.cfg.load_www(self.filename, dict(www=dict(port=9888)))
        self.assertResults(
            www=dict(
                port=9888,
                plugins={},
    ...
    def test_load_www_plugin(self):
        self.cfg.load_www(self.filename, dict(www=dict(plugins={"waterfall": {"foo": "bar"}})))
        self.assertResults(
            www=dict(
                port=None,
    ...
    def test_load_www_allowed_origins(self):
        self.cfg.load_www(self.filename, dict(www=dict(allowed_origins=["a", "b"])))
        self.assertResults(
            www=dict(
                port=None,
                allowed_origins=["a", "b"],
    ...
    def test_load_www_logfileName(self):
        self.cfg.load_www(self.filename, dict(www=dict(logfileName="http-access.log")))
        self.assertResults(
            www=dict(
                port=None,
                plugins={},
    ...
    def test_load_services_nominal(self):
        ...
        self.cfg.load_services(self.filename, dict(services=[myService]))
    ...
    def test_load_services_duplicate(self):
        with capture_config_errors() as errors:
            ...
            self.cfg.load_services(
                self.filename, dict(services=[MyService(x="a"), MyService(x="b")])
            )
