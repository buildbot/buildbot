...
class FakeDataConnector(service.AsyncMultiService):
    ...
    def get(self, path, filters=None, fields=None, order=None, limit=None, offset=None, properties=None):
        if not isinstance(path, tuple):
            raise TypeError('path must be a tuple')
        return self.realConnector.get(
                path, filters=filters, fields=fields, order=order, limit=limit, offset=offset, properties=None,
        )
