...
class ShellMaster(RunMasterBase):
    ...
    def test_logs(self):
        yield self.setupConfig(
            self.config_for_master_command(command=[sys.executable, "-c", 'print("hello")'])
        )
    ...
    def test_fails(self):
        yield self.setupConfig(
            self.config_for_master_command(command=[sys.executable, "-c", "exit(1)"])
        )
    ...
    def test_interrupt(self):
        yield self.setupConfig(
            self.config_for_master_command(
