...
class FakeWorkerBuilder(pb.Referenceable):

    """
    Fake worker-side WorkerBuilder object
    """


class FakeWorkerBuildWorker(pb.Referenceable):

    """
    Fake worker-side BuildWorker object
    ...
    def remote_setBuilderList(self, builder_info):
        ...
        slbuilders = [FakeWorkerBuilder() for n in builder_names]
...
class MyBuildWorker(buildworker.BuildWorker):

    def attached(self, conn):
        self.detach_d = defer.Deferred()
        return buildworker.BuildWorker.attached(self, conn)

    def detached(self):
        buildworker.BuildWorker.detached(self)
...
class TestWorkerComm(unittest.TestCase):
    ...
    @ivar buildworker: master-side L{BuildWorker} instance
    @ivar workerbuildworker: worker-side L{FakeWorkerBuildWorker} instance
    ...
    def setUp(self):
        ...
        yield self.master.buildworkers.disownServiceParent()
        self.buildworkers = self.master.buildworkers = bworkermanager.BuildworkerManager(self.master)
        ...
        self.master.data.updates.buildworkerConfigured = lambda *a, **k: None
        yield self.master.startService()

        self.buildworker = None
        self.port = None
        self.workerbuildworker = None
        self.connector = None
        self._detach_deferreds = []
    ...
    def tearDown(self):
        ...
        deferreds = self._detach_deferreds + [
            ...
            self.buildworkers.stopService(),
        ]

        # if the buildworker is still attached, wait for it to detach, too
    ...
    def addWorker(self, **kwargs):
        ...
        @param **kwargs: arguments to pass to the L{BuildWorker} constructor.
        """
        self.buildworker = MyBuildWorker("testworker", "pw", **kwargs)
        ...
        yield self.buildworkers.reconfigServiceWithBuildbotConfig(new_config)
    ...
    def connectWorker(self, waitForBuilderList=True):
        ...
        been called
        @returns: L{FakeWorkerBuildWorker} and a Deferred that will fire when it
        is detached; via deferred
        """
        factory = pb.PBClientFactory()
        creds = credentials.UsernamePassword("testworker", "pw")
        setBuilderList_d = defer.Deferred()
        workerbuildworker = FakeWorkerBuildWorker(
            lambda: setBuilderList_d.callback(None))

        login_d = factory.login(creds, workerbuildworker)

        @login_d.addCallback
        def logged_in(persp):
            workerbuildworker.setMasterPerspective(persp)

            # set up to hear when the worker side disconnects
            workerbuildworker.detach_d = defer.Deferred()
            persp.broker.notifyOnDisconnect(lambda:
                                            workerbuildworker.detach_d.callback(None))
            self._detach_deferreds.append(workerbuildworker.detach_d)

            return workerbuildworker
        ...
        if not waitForBuilderList:
            ...
            d.addCallback(lambda _: workerbuildworker)
            return d
