...
class Tests(RunFakeMasterTestCase):
    ...
    def create_single_worker_two_builder_lock_config(
        self, lock_cls: type[MasterLock] | type[WorkerLock], mode: str
    ) -> InlineCallbacksType[tuple[list[BuildStepController], list[int]]]:
    ...
    @defer.inlineCallbacks
    def create_two_worker_two_builder_lock_config(
        self, mode: str
    ) -> InlineCallbacksType[tuple[list[BuildStepController], list[int]]]:
        stepcontrollers = [BuildStepController(), BuildStepController()]
    ...
    @defer.inlineCallbacks
    def assert_two_builds_created_one_after_another(
        self, stepcontrollers: list[BuildStepController], builder_ids: list[int]
    ...
    @defer.inlineCallbacks
    def assert_two_steps_created_one_after_another(
        self, stepcontrollers: list[BuildStepController], builder_ids: list[int]
    ) -> InlineCallbacksType[None]:
        # start two builds and verify that a second build starts after the
        # first is finished
    ...
    @defer.inlineCallbacks
    def test_builder_lock_prevents_concurrent_builds(
        self, lock_cls: type[MasterLock] | type[WorkerLock], mode: str
    ) -> InlineCallbacksType[None]:
        """
    ...
    @defer.inlineCallbacks
    def test_step_lock_prevents_concurrent_builds(
        self, lock_cls: type[MasterLock] | type[WorkerLock], mode: str
    ) -> InlineCallbacksType[None]:
        """
        Tests whether a builder lock works at all in preventing a build when
    ...
    @defer.inlineCallbacks
    def test_builder_lock_release_wakes_builds_for_another_builder(
        self, mode: str
    ) -> InlineCallbacksType[None]:
        """
...
class TestReconfig(RunFakeMasterTestCase):
    def create_stepcontrollers(
        self, count: int, lock: MasterLock | WorkerLock | None, mode: str | None
    ) -> list[BuildStepController]:
        stepcontrollers = []
        for _ in range(count):
            locks = [lock.access(mode)] if lock is not None else []  # type: ignore[arg-type]
    ...
    def update_builder_config(
        self,
        config_dict: dict[str, Any],
        stepcontrollers: list[BuildStepController],
        lock: MasterLock | WorkerLock | None,
        mode: str | None,
    ) -> None:
    ...
    @defer.inlineCallbacks
    def create_single_worker_n_builder_lock_config(
        self,
        builder_count: int,
        lock_cls: type[MasterLock] | type[WorkerLock],
        max_count: int,
        mode: str,
    ) -> InlineCallbacksType[
        tuple[list[BuildStepController], dict[str, Any], MasterLock | WorkerLock, list[int]]
    ]:
        ...
        for i in range(builder_count):
            builder_ids.append((yield self.master.data.updates.findBuilderId(f'builder{i}')))
    ...
    def test_changing_max_lock_count_does_not_break_builder_locks(
        ...
        lock_cls: type[MasterLock] | type[WorkerLock],
        mode: str,
        max_count_before: int,
        max_count_after: int,
        allowed_builds_before: int,
        allowed_builds_after: int,
    ) -> InlineCallbacksType[None]:
        """
    ...
    def test_changing_max_lock_count_does_not_break_step_locks(
        ...
        max_count_before: int,
        max_count_after: int,
        allowed_steps_before: int,
        allowed_steps_after: int,
    ) -> InlineCallbacksType[None]:
        """
        Check that Buildbot does not allow extra claims on a claimed lock after
        a reconfig that changed the maxCount of that lock. Some Buildbot
