...
from buildbot.buildworker import AbstractLatentBuildWorker
...
class DockerLatentBuildWorker(AbstractLatentBuildWorker):
    instance = None
    ...
    def __init__(self, name, password, docker_host, image=None, command=None,
        ...
        if not client:
            config.error("The python module 'docker-py' is needed to use a"
                         " DockerLatentBuildWorker")
        if not image and not dockerfile:
            config.error("DockerLatentBuildWorker: You need to specify at least"
    ...
    def _thd_start_instance(self):
        ...
        if (not self._image_exists(docker_client, image)):
            ...
            raise LatentBuildWorkerFailedToSubstantiate(
