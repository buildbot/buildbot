...
class LocalBuildWorker(BuildWorker):
    ...
    def checkConfig(self, name, workdir=None, usePty=False, **kwargs):
        BuildWorker.checkConfig(self, name, None, **kwargs)
        self.LocalBuildWorkerFactory = None
        try:
            # importing here to avoid dependency on buildbot worker package
            from buildworker.bot import LocalBuildWorker as RemoteLocalBuildWorker
            self.LocalBuildWorkerFactory = RemoteLocalBuildWorker
        except ImportError:
            error("LocalBuildWorker needs the buildbot-worker package installed (pip install buildbot-worker)")
