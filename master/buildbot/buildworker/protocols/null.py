...
class Connection(base.Connection):
    ...
    def remotePrint(self, message):
        return defer.maybeDeferred(self.buildworker.bot.remote_print, message)

    def remoteGetWorkerInfo(self):
        return defer.maybeDeferred(self.buildworker.bot.remote_getWorkerInfo)

    def remoteSetBuilderList(self, builders):
        return defer.maybeDeferred(self.buildworker.bot.remote_setBuilderList, builders)

    def remoteStartCommand(self, remoteCommand, builderName, commandId, commandName, args):
        remoteCommand = RemoteCommandProxy(remoteCommand)
        args = self.createArgsProxies(args)
        workerbuilder = self.buildworker.bot.builders[builderName]
        return defer.maybeDeferred(workerbuilder.remote_startCommand, remoteCommand,
                                   commandId, commandName, args)

    def remoteShutdown(self):
        return defer.maybeDeferred(self.buildworker.stopService)

    def remoteStartBuild(self, builderName):
        return defer.succeed(self.buildworker.bot.builders[builderName].remote_startBuild())

    def remoteInterruptCommand(self, builderName, commandId, why):
        workerbuilder = self.buildworker.bot.builders[builderName]
        return defer.maybeDeferred(workerbuilder.remote_interruptCommand, commandId, why)
