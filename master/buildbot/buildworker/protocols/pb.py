...
class Listener(base.Listener):
    ...
    @defer.inlineCallbacks
    def _getPerspective(self, mind, buildworkerName):
        bworkers = self.master.buildworkers
        log.msg("worker '%s' attaching from %s" % (buildworkerName,
        ...
        buildworker = bworkers.getBuildworkerByName(buildworkerName)
        conn = Connection(self.master, buildworker, mind)

        # inform the manager, logging any problems in the deferred
        accepted = yield bworkers.newConnection(conn, buildworkerName)
...
class Connection(base.Connection, pb.Avatar):
    ...
    def __init__(self, master, buildworker, mind):
    ...
    def attached(self, mind):
        ...
        # buildworker
        yield self.buildworker.attached(self)
    ...
    def remoteGetWorkerInfo(self):
        ...
        if "worker_commands" in info:
            defer.returnValue(info)
        try:
            info["worker_commands"] = yield self.mind.callRemote('getCommands')
        except pb.NoSuchMethod:
            log.msg("BuildWorker.getCommands is unavailable - ignoring")

        try:
            info["version"] = yield self.mind.callRemote('getVersion')
        except pb.NoSuchMethod:
            log.msg("BuildWorker.getVersion is unavailable - ignoring")
    ...
    def remoteStartCommand(self, remoteCommand, builderName, commandId, commandName, args):
        workerbuilder = self.builders.get(builderName)
        remoteCommand = RemoteCommand(remoteCommand)
        args = self.createArgsProxies(args)
        return workerbuilder.callRemote('startCommand',
    ...
    def remoteShutdown(self):
        ...
        def old_way():
            d = None
            for b in itervalues(self.buildworker.workerbuilders):
                if b.remote:
                    d = b.mind.callRemote("shutdown")
                    break

            if d:
                name = self.buildworker.workername
    ...
    def remoteStartBuild(self, builderName):
        workerbuilder = self.builders.get(builderName)
        return workerbuilder.callRemote('startBuild')

    def remoteInterruptCommand(self, builderName, commandId, why):
        workerbuilder = self.builders.get(builderName)
        return defer.maybeDeferred(workerbuilder.callRemote, "interruptCommand",
                                   commandId, why)

    # perspective methods called by the worker

    def perspective_keepalive(self):
        self.buildworker.messageReceivedFromWorker()

    def perspective_shutdown(self):
        self.buildworker.messageReceivedFromWorker()
        self.buildworker.shutdownRequested()
