...
from buildbot.interfaces import ILatentBuildWorker
from buildbot.interfaces import LatentBuildWorkerFailedToSubstantiate
...
class AbstractBuildWorker(service.BuildbotService, object):
    ...
    I represent a build worker -- a remote machine capable of
    running builds.  I am instantiated by the configuration file, and can be
    subclassed to add extra functionality."""

    implements(IBuildWorker)
    ...
    def checkConfig(self, name, password, max_builds=None,
        ...
        @param max_builds: maximum number of simultaneous builds that will
                           be run concurrently on this buildworker (the
        ...
        self.properties.update(properties, "BuildWorker")
        self.properties.setProperty("workername", name, "BuildWorker")
    ...
    def _getWorkerInfo(self):
        buildworker = yield self.master.data.get(
            ('buildworkers', self.buildworkerid))
        self._applyWorkerInfo(buildworker['workerinfo'])
    ...
    def startService(self):
        ...
        self.buildworkerid = yield self.master.data.updates.findBuildworkerId(
            self.name)
    ...
    def reconfigService(self, name, password, max_builds=None,
                        ...
                        properties=None, locks=None, keepalive_interval=3600):
        # Given a BuildWorker config arguments, configure this one identically.
        # Because BuildWorker objects are remotely referenced, we can't replace them
        ...
        # update our records with the buildworker manager
        if not self.registration:
            self.registration = yield self.master.buildworkers.register(self)
        ...
        yield self.master.data.updates.buildworkerConfigured(self.buildworkerid, self.master.masterid, bids)
    ...
    def _missing_timer_fired(self):
        ...
        text += ("has noticed that the buildworker named %s went away\n" %
                 self.name)
        text += "\n"
        text += ("It last disconnected at %s (buildmaster-local time)\n" %
                 time.ctime(time.time() - self.missing_timeout))  # approx
        text += "\n"
        text += "The admin on record (as reported by BUILDWORKER:info/admin)\n"
        ...
        subject = "Buildbot: buildworker %s was lost" % (self.name,)
    ...
    def attached(self, conn):
        ...
        metrics.MetricCountEvent.log("AbstractBuildWorker.attached_workers", 1)
        ...
        yield self.master.data.updates.buildworkerConnected(
            buildworkerid=self.buildworkerid,
    ...
    def detached(self):
        ...
        log.msg("BuildWorker.detached(%s)" % (self.name,))
        self.master.status.workerDisconnected(self.name)
        self.releaseLocks()
        yield self.master.data.updates.buildworkerDisconnected(
            buildworkerid=self.buildworkerid,
            masterid=self.master.masterid,
        )
    ...
    def addWorkerBuilder(self, sb):
        self.workerbuilders[sb.builder_name] = sb

    def removeWorkerBuilder(self, sb):
        try:
            del self.workerbuilders[sb.builder_name]
        except KeyError:
            pass
    ...
    def canStartBuild(self):
        """
        I am called when a build is requested to see if this buildworker
        can start a build.  This function can be used to limit overall
        concurrency on the buildworker.
        ...
        if self.max_builds:
            active_builders = [sb for sb in itervalues(self.workerbuilders)
                               if sb.isBusy()]
    ...
    def _mail_missing_message(self, subject, text):
        ...
        for st in buildmaster.services:
            ...
            log.msg("buildworker-missing msg using default MailNotifier")
...
class BuildWorker(AbstractBuildWorker):

    def sendBuilderList(self):
        d = AbstractBuildWorker.sendBuilderList(self)
        ...
        def _set_failed(why):
            log.msg("BuildWorker.sendBuilderList (%s) failed" % self)
            log.err(why)
    ...
    def detached(self):
        AbstractBuildWorker.detached(self)
        self.botmaster.workerLost(self)
        self.startMissingTimer()

    def buildFinished(self, sb):
        """This is called when a build on this worker is finished."""
        AbstractBuildWorker.buildFinished(self, sb)
...
class AbstractLatentBuildWorker(AbstractBuildWorker):

    """A build worker that will start up a worker instance when needed.

    To use, subclass and implement start_instance and stop_instance.

    See ec2buildworker.py for a concrete example.  Also see the stub example in
    test/test_workers.py.
    """

    implements(ILatentBuildWorker)
    ...
    def __init__(self, name, password,
                 ...
                 **kwargs):
        AbstractBuildWorker.__init__(self, name, password, **kwargs)
    ...
    def failed_to_start(self, instance_id, instance_state):
        ...
        raise LatentBuildWorkerFailedToSubstantiate(instance_id, instance_state)
    ...
    def attached(self, bot):
        ...
        return AbstractBuildWorker.attached(self, bot)
    ...
    def _substantiation_failed(self, failure):
        ...
        status = buildmaster.getStatus()
        text = "The Buildbot working for '%s'\n" % status.getTitle()
        text += ("has noticed that the latent buildworker named %s \n" %
                 self.name)
        ...
        text += " The Buildbot\n"
        text += " %s\n" % status.getTitleURL()
        subject = "Buildbot: buildworker %s never substantiated" % (self.name,)
        return self._mail_missing_message(subject, text)

    def canStartBuild(self):
        if self.insubstantiating:
            return False
        return AbstractBuildWorker.canStartBuild(self)
    ...
    def _soft_disconnect(self, fast=False):
        ...
        if self.build_wait_timeout < 0:
            yield AbstractBuildWorker.disconnect(self)
            return
        ...
        if self.conn is not None:
            ...
            # in BotMaster.removeWorker, *or* when a new worker requests a
            ...
            yield defer.DeferredList([
                AbstractBuildWorker.disconnect(self),
    ...
    def stopService(self):
        res = defer.maybeDeferred(AbstractBuildWorker.stopService, self)
        if self.conn is not None:
    ...
    def updateWorker(self):
        ...
        for b in self.botmaster.getBuildersForWorker(self.name):
            if b.name not in self.workerbuilders:
                b.addLatentWorker(self)
        return AbstractBuildWorker.updateWorker(self)
    ...
    def sendBuilderList(self):
        ...
        def _sent(slist):
            ...
            for name in slist:
                ...
                # workerbuilders out of a bit of paranoia.
                b = self.botmaster.builders.get(name)
                sb = self.workerbuilders.get(name)
                if b and sb:
