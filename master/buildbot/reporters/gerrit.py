...
class GerritStatusPush(service.BuildbotService):
    ...
    def reconfigService(self, server, username=None, reviewCB=DEFAULT_REVIEW,
                        ...
                        wantSteps=False, wantLogs=False, auth=None):
        ...
        if auth != None:
            self._http = yield httpclientservice.HTTPClientService.getService(
                self.master, base_url=self.gerrit_server, debug=True, auth=auth)
    ...
    def sendCodeReviews(self, build, result):
        ...
        if getProperty(build, "event.change.id") is not None:
            ...
            if project is not None and revision is not None:
                self.sendCodeReview(project, revision, result, getProperty(build, "event.change.id"))
                return

    @defer.inlineCallbacks
    def sendCodeReview(self, project, revision, result, change_id=None):
        if self._http == None:
            ...
            if gerrit_version >= LooseVersion("2.13"):
            ...
            if labels:
                for label, value in labels.items():
                    payload['labels'][label] = value

            url = '/changes/{}/revisions/{}/review'.format(change_id, revision)
            response = yield self._http.post(url, json=payload)

            responseCode = yield response.code
            if responseCode < 200 or responseCode >= 300:
                responseContent = yield response.content()
                raise Exception('Gerrit replied with {}: {}'.format(responseCode, responseContent))
