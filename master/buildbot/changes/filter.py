...
class ChangeFilter(ComparableMixin):
    ...
    def __init__(self,
            ...
            tags=None, tags_re=None, tags_fn=None):
        def mklist(x):
        ...
        self.checks = [
                ...
                (mklist(tags), mkre(tags_re), tags_fn, "tags"),
            ]
    ...
    def filter_change(self, change):
        ...
        for (filt_list, filt_re, filt_fn, chg_attr) in self.checks:
            ...
            # Value could be a string or a list. Let's handle it.
            if chg_val is None or isinstance(chg_val, basestring):
                ...
                if filt_re is not None:
                    for val in chg_val:
                        if val is not None and filt_re.match(val):
                            break
                    else:
                        return False
            # Call the filter function only once, since it could implement some
            # complicated logic based on multiple values if given.
    ...
    def __repr__(self):
        checks = []
        for (filt_list, filt_re, filt_fn, chg_attr) in self.checks:
            # Note: tags is a list, but since we do not do eval(repr(objChangeFilter)),
            # we are fine with this implementation. It is unambiguous and readable.
