...
class SourceStampFilter(ComparableMixin):
    ...
    def __init__(
        ...
        self.project_filters = self.create_filters(
            project_eq, project_not_eq, project_re, project_not_re, "project"
        )
        self.codebase_filters = self.create_filters(
            codebase_eq, codebase_not_eq, codebase_re, codebase_not_re, "codebase"
        )
        self.repository_filters = self.create_filters(
            repository_eq,
            repository_not_eq,
            repository_re,
            repository_not_re,
        ...
        self.branch_filters = self.create_branch_filters(
            branch_eq, branch_not_eq, branch_re, branch_not_re, "branch"
        )
    ...
    def is_matched(self, ss):
        ...
        if self.codebase_filters and not self.do_prop_match(ss, "codebase", self.codebase_filters):
            return False
        if self.repository_filters and not self.do_prop_match(
            ss, "repository", self.repository_filters
        ):
            return False
