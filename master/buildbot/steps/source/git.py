...
class Git(Source):
    ...
    def __init__(self, repourl=None, branch='HEAD', mode='incremental', method=None,
        ...
        @param sshPrivateKey: The private key to use when running git.
    ...
    def _getSshPrivateKeyWorkDir(self):
    ...
    def _getSshPrivateKeyRelPath(self):
        ...
        basename = self.build.path_module.basename(
                self._getSshPrivateKeyWorkDir().rstrip('/\\'))
        basename = '.{0}.buildbot-ssh-key'.format(basename)
    ...
    def _downloadSshPrivateKeyIfNeeded(self):
        ...
        yield self._downloadFile(privateKey, self._getSshPrivateKeyRelPath(),
                                 workdir=self._getSshPrivateKeyWorkDir(), mode=0o400)
    ...
    def _removeSshPrivateKeyIfNeeded(self):
        ...
        keyPath = self.build.path_module.join(self._getSshPrivateKeyWorkDir(),
                                              self._getSshPrivateKeyRelPath())
        cmd = remotecommand.RemoteCommand('rmdir', {'dir': keyPath,
                                                    'logEnviron': self.logEnviron})
        cmd.useLog(self.stdio_log, False)
        yield self.runCommand(cmd)
        defer.returnValue(RC_SUCCESS)
