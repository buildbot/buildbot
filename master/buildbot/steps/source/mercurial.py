...
class Mercurial(Source):
    ...
    async def mode_incremental(self):
        ...
        if updatable:
            await self._dovccmd(self.getHgPullCommand())
        else:
            await self._clone()

        await self._checkBranchChange()

    async def clean(self):
        command = ['--config', 'extensions.purge=', 'purge']
        await self._dovccmd(command)
        await self._pullUpdate()
    ...
    async def parseGotRevision(self):
        stdout = await self._dovccmd(['parents', '--template', '{node}\\n'], collectStdout=True)
    ...
    async def _checkBranchChange(self):
        current_branch = await self._getCurrentBranch()
        msg = (
        ...
        msg += ' Updating.'
        log.msg(msg)
        await self._removeAddedFilesAndUpdate(None)
    ...
    async def _getCurrentBranch(self):
        if self.branchType == 'dirname':
            return self.branch
        stdout = await self._dovccmd(['identify', '--branch'], collectStdout=True)
        return stdout.strip()
    ...
    async def _removeAddedFilesAndUpdate(self, _):
        command = ['locate', 'set:added()']
        stdout = await self._dovccmd(command, collectStdout=True, decodeRC={0: SUCCESS, 1: SUCCESS})
