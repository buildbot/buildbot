...
class P4(Source):
    ...
    def __init__(
            self, mode='incremental', method=None, p4base=None, p4branch=None,
            p4port=None, p4user=None, p4passwd=None, p4extra_views=(),
            p4line_end='local', p4viewspec=None, p4viewspec_suffix='...',
            p4client=Interpolate(
                'buildbot_%(prop:slavename)s_%(prop:buildername)s'),
            p4client_spec_options='allwrite rmdir', p4extra_args=None,
            p4bin='p4', p4branch_is_a_stream=False, use_tickets=False, **
            kwargs):
        self.method = method
        self.mode = mode
        ...
        if self.mode not in self.possible_modes and not interfaces.IRenderable.providedBy(self.mode):
            config.error(
                "mode %s is not an IRenderable, or one of %s" %
    ...
    def mode_full(self, _):
        ...
        if self.revision:
            if debug_logging:
                log.msg(
                    "P4: full() sync command based on :base:%s changeset:%d",
            ...
            if debug_logging:
                log.msg(
                    "P4: full() sync command based on :base:%s no revision",
                    self._getP4BaseForLog())
    ...
    def mode_incremental(self, _):
        ...
        if self.revision:
            command.extend(
                ['%s...@%d' %
                 (self._getP4BaseForCommand(),
                  int(self.revision))])

        if debug_logging:
            log.msg("P4:incremental() command:%s revision:%s",
        ...
        if self.p4viewspec:
            ...
            for k, v in self.p4viewspec:
                ...
                client_spec += '\t%s%s%s%s %s//%s/%s%s%s\n' % (
                    qa, k, suffix, qa, qb, self.p4client, v, suffix, qb)
        else:
