        ...
        if self.maxsize is not None:
            for arg in args:
                if len(arg) > self.maxsize:
                    raise ValidationError(
                        f"{self.label}: is too large {len(arg)} > {self.maxsize}"
                    )
...
class CodebaseParameter(NestedParameter):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        fields_dict = dict(
            branch=branch, revision=revision, repository=repository, project=project
        )
...
class ForceScheduler(base.BaseScheduler):
    ...
    def __init__(
        ...
        if self.checkIfType(username, BaseParameter):
            ...
            config.error(
                f"ForceScheduler '{name}': username must be a StringParameter: "
                f"{repr(username)}"
            )
