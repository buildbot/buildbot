...
class BaseParameter(object):
    ...
    def fullName(self):
        ...
        # join with '_' if both are set (cannot put '.', because it is used as **kwargs)
...
class ChoiceStringParameter(BaseParameter):
    ...
    def parse_from_arg(self, s):
        if self.strict and s not in self.choices:
            raise ValidationError("'%s' does not belong to list of available choices '%s'" % (s, self.choices))
        return s
...
class BuildworkerChoiceParameter(ChoiceStringParameter):

    """A parameter that lets the buildworker name be explicitly chosen.
    ...
    there is no buildworker preference.
    """
    anySentinel = '-any-'
    label = 'Build worker'
    required = False
