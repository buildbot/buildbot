...
WWW_PKGS := www/base www/react-base www/console_view www/grid_view www/react-grid_view www/waterfall_view www/wsgi_dashboards www/badges
...
frontend_tests_headless: frontend_deps
	for i in $(WWW_PKGS); \
		do (cd $$i; $(YARN) install --pure-lockfile); done
	for i in $(WWW_PKGS_FOR_UNIT_TESTS); \
		do (cd $$i; $(YARN) run build-dev || exit 1; $(YARN) run test $$(if [ $$i != "www/react-base" -a $$i != "www/react-data-module" -a $$i != "www/react-ui" -a $$i != "www/react-grid_view" ]; then echo --browsers BBChromeHeadless; fi) || exit 1) || exit 1; done
