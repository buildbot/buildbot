...
class BuildworkerTimeoutError(Exception):
    pass
...
class BuildWorkerDetectedError(Exception):
    pass
...
class LogWatcher(LineOnlyReceiver):
    ...
    def lineReceived(self, line):
        ...
        if "loading configuration from" in line:
            self.in_reconfig = True
        if "Creating BuildWorker" in line:
            self.processtype = "buildworker"

        if self.in_reconfig:
            log.msg(line)

        if "message from master: attached" in line:
            return self.finished("buildworker")
