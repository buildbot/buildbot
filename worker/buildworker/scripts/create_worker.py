...
# note: this line is matched against to check that this is a buildworker
...
class CreateWorkerError(Exception):
    ...
    """
    Raised on errors while setting up buildworker directory.
    """


def _makeBaseDir(basedir, quiet):
    """
    Make buildworker base directory if needed.

    @param basedir: buildworker base directory relative path
...
def createWorker(config):
    ...
    try:
        ...
        log.msg("%s\nfailed to configure buildworker in %s" %
                (exception, config['basedir']))
        return 1

    if not quiet:
        log.msg("buildworker configured in %s" % basedir)
