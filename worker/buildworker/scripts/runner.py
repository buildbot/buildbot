...
class StartOptions(MakerBase):
    subcommandFunction = "buildworker.scripts.start.startCommand"
    optFlags = [
    ...
    def getSynopsis(self):
        return "Usage:    buildworker start [<basedir>]"


class StopOptions(MakerBase):
    subcommandFunction = "buildworker.scripts.stop.stop"

    def getSynopsis(self):
        return "Usage:    buildworker stop [<basedir>]"


class RestartOptions(MakerBase):
    subcommandFunction = "buildworker.scripts.restart.restart"
    optFlags = [
        ['quiet', 'q', "Don't display startup log messages"],
        ['nodaemon', None, "Don't daemonize (stay in foreground)"],
    ]
    ...
    def getSynopsis(self):
        return "Usage:    buildworker restart [<basedir>]"


class UpgradeWorkerOptions(MakerBase):
    subcommandFunction = "buildworker.scripts.upgrade_worker.upgradeWorker"
    optFlags = [
    ]
    optParameters = [
    ]
    ...
    def getSynopsis(self):
        return "Usage:    buildworker upgrade-worker [<basedir>]"

    longdesc = """
    This command takes an existing buildworker working directory and
    upgrades it to the current version.
    """


class CreateWorkerOptions(MakerBase):
    subcommandFunction = "buildworker.scripts.create_worker.createWorker"
    optFlags = [
    ...
    optParameters = [
        ...
        ["allow-shutdown", "a", None,
         "Allows the buildworker to initiate a graceful shutdown. One of "
         "'signal' or 'file'"]
    ]

    longdesc = """
    This command creates a buildworker working directory and buildbot.tac
    ...
    def getSynopsis(self):
        return "Usage:    buildworker create-worker " \
...
class Options(usage.Options):
    synopsis = "Usage:    buildworker <command> [command options]"
    ...
    subCommands = [
        ...
        ['upgrade-worker', None, UpgradeWorkerOptions,
         "Upgrade an existing buildworker directory for the current version"],
        ['start', None, StartOptions, "Start a buildworker"],
        ['stop', None, StopOptions, "Stop a buildworker"],
    ...
    def opt_version(self):
        import buildworker
        log.msg("Buildworker version: %s" % buildworker.version)
