...
class Follower(object):
    ...
    def follow(self):
        ...
        from buildworker.scripts.logwatcher import LogWatcher
        self.rc = 0
    ...
    def _failure(self, why):
        from twisted.internet import reactor
        from buildworker.scripts.logwatcher import BuildmasterTimeoutError, \
            ReconfigError, BuildworkerTimeoutError, BuildWorkerDetectedError
        if why.check(BuildmasterTimeoutError):
            log.msg("""
The buildworker took more than 10 seconds to start, so we were unable to
confirm that it started correctly. Please 'tail twistd.log' and look for a
line that says 'configuration update complete' to verify correct startup.
""")
        elif why.check(BuildworkerTimeoutError):
            log.msg("""
The buildworker took more than 10 seconds to start and/or connect to the
buildworker, so we were unable to confirm that it started and connected
...
messages like 'will retry in 6 seconds', your buildworker might not have the
correct hostname or portnumber for the buildworker, or the buildworker might
not be running. If you see messages like
   'Failure: twisted.cred.error.UnauthorizedLogin'
then your buildworker might be using the wrong botname or password. Please
correct these problems and then restart the buildworker.
""")
        elif why.check(ReconfigError):
            log.msg("""
The buildworker appears to have encountered an error in the master.cfg config
file during startup. It is probably running with an empty configuration right
now. Please inspect and fix master.cfg, then restart the buildworker.
""")
        elif why.check(BuildWorkerDetectedError):
            log.msg("""
Buildworker is starting up, not following logfile.
""")
        else:
            log.msg("""
Unable to confirm that the buildworker started correctly. You may need to
