...
WORKER_RUNNER=/usr/bin/buildworker


# Source buildworker configuration
[[ -r /etc/default/buildworker ]] && . /etc/default/buildworker
#[[ -r /etc/sysconfig/buildworker ]] && . /etc/sysconfig/buildworker

# Or define/override the configuration here
#WORKER_ENABLED[1]=0                    # 0-enabled, other-disabled
#WORKER_NAME[1]="buildworker #1"         # short name printed on start/stop
...
function worker_config_valid() {
    # Function validates buildworker instance startup variables based on array
    # index
    local errors=0
    local index=$1

    if ! is_enabled "${WORKER_ENABLED[$index]}" && ! is_disabled "${WORKER_ENABLED[$index]}" ; then
        log_warning_msg "buildworker #${index}: invalid enabled status"
        errors=$(($errors+1))
    fi

    if [[ -z ${WORKER_NAME[$index]} ]]; then
        log_failure_msg "buildworker #${index}: no name"
        errors=$(($errors+1))
    fi

    if [[ -z ${WORKER_USER[$index]} ]]; then
        log_failure_msg "buildworker #${index}: no run user specified"
        errors=$( ($errors+1) )
    elif ! getent passwd ${WORKER_USER[$index]} >/dev/null; then
        log_failure_msg "buildworker #${index}: unknown user ${WORKER_USER[$index]}"
        errors=$(($errors+1))
    fi

    if [[ ! -d "${WORKER_BASEDIR[$index]}" ]]; then
        log_failure_msg "buildworker ${index}: basedir does not exist ${WORKER_BASEDIR[$index]}"
        errors=$(($errors+1))
    fi
...
function check_config() {
    ...
    for i in $( seq ${#WORKER_ENABLED[@]} ); do
        if is_disabled "${WORKER_ENABLED[$i]}" ; then
            log_warning_msg "buildworker #${i}: disabled"
            continue
        fi
...
case "$1" in
    start)
        do_op "worker_op" "start" "Starting buildworker" "$2"
        exit $?
        ;;
    stop)
        do_op "worker_op" "stop" "Stopping buildworker" "$2"
        exit $?
        ;;
    reload)
        do_op "worker_op" "reload" "Reloading buildworker" "$2"
        exit $?
        ;;
    restart|force-reload)
        do_op "worker_op" "restart" "Restarting buildworker" "$2"
        exit $?
        ;;
    *)
