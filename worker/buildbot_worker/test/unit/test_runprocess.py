...
class TestRunProcess(BasedirMixin, unittest.TestCase):
    ...
    def testCommandEncoding(self):
        s = runprocess.RunProcess("abcd", self.basedir, "utf-8", self.send_update)
        self.assertIsInstance(s.command, bytes)
        self.assertIsInstance(s.fake_command, bytes)

    def testCommandEncodingList(self):
        s = runprocess.RunProcess(["abcd", b"efg"], self.basedir, "utf-8", self.send_update)
    ...
    def testCommandEncodingObfuscated(self):
        s = runprocess.RunProcess(
            [bsutil.Obfuscated("abcd", "ABCD")], self.basedir, "utf-8", self.send_update
        )
        self.assertIsInstance(s.command[0], bytes)
        self.assertIsInstance(s.fake_command[0], bytes)
    ...
    def testObfuscatedCommand(self):
        s = runprocess.RunProcess(
            [("obfuscated", "abcd", "ABCD")], self.basedir, "utf-8", self.send_update
        )
    ...
    def testMultiWordStringCommand(self):
        # careful!  This command must execute the same on windows and UNIX
        s = runprocess.RunProcess(
            "echo Happy Days and Jubilation", self.basedir, "utf-8", self.send_update
        )
    ...
    def testInitialStdinUnicode(self):
        s = runprocess.RunProcess(
            catCommand(), self.basedir, "utf-8", self.send_update, initialStdin="hello"
        )
    ...
    def testTrickyArguments(self):
        ...
        s = runprocess.RunProcess(
            printArgsCommand() + args, self.basedir, "utf-8", self.send_update
        )
        yield s.start()
    ...
    def testCommandTimeout(self):
        s = runprocess.RunProcess(
            sleepCommand(10), self.basedir, "utf-8", self.send_update, timeout=5
        )
        clock = task.Clock()
        s._reactor = clock
    ...
    def testCommandMaxTime(self):
        s = runprocess.RunProcess(
            sleepCommand(10), self.basedir, "utf-8", self.send_update, maxTime=5
        )
        clock = task.Clock()
        s._reactor = clock
    ...
    def testEnvironPythonPath(self):
        ...
        self.assertFalse(
            re.match("\bPYTHONPATH=a{0}".format(os.pathsep), headers),
            "got:\n" + headers,
        )
