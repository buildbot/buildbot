...
class RunProcessPP(protocol.ProcessProtocol):
    ...
    def connectionMade(self):
        ...
        if self.command.useProcGroup:
            if self.debug:
                log.msg(" recording pid {0} as subprocess pgid".format(self.transport.pid))
...
class RunProcess(object):
    ...
    def __init__(
        ...
        assert usePTY in (
            True,
            False,
        ), "Unexpected usePTY argument value: {!r}. Expected boolean.".format(usePTY)
        self.usePTY = usePTY
    ...
    def _startCommand(self):
        ...
        if self.logEnviron:
            ...
            for name in env_names:
                msg += "  {0}={1}\n".format(
                    bytes2unicode(name, encoding=self.unicode_encoding),
                    bytes2unicode(self.environ[name], encoding=self.unicode_encoding),
                )
    ...
    def _spawnAsBatch(self, processProtocol, executable, args, env, path, usePTY):
        ...
        if PY3:
            ...
            tf = NamedTemporaryFile(mode="w+", dir=".", suffix=".bat", delete=False)
    ...
    def doTimeout(self):
        self.ioTimeoutTimer = None
        msg = "command timed out: {0} seconds without output running {1}".format(
    ...
    def sendSig(self, interruptSignal):
        ...
        if not hit and self.useProcGroup and runtime.platformType == "posix":
            ...
            if sig is None:
                ...
                log.msg("trying to kill process group {0}".format(self.process.pgid))
                try:
        ...
        if not hit:
            try:
                log.msg("trying process.signalProcess('{0}')".format(interruptSignal))
