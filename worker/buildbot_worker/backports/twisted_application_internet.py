...
# Everything below this is a verbatim copy of twisted.application.internet
# from Twisted 16.4.1, except where explicitely mentioned
# (search for 'Buildbot' in comments)
...
for tran in 'TCP UNIX SSL UDP UNIXDatagram Multicast'.split():
    for side in 'Server Client'.split():
        ...
        # Buildbot: in Twisted < 15, the base class is not a new style class.
        # Therefore, it can't be suclassed directly using type(), but it's
        # enough to add ``object`` to solve that issue.
        klass = type(tran+side, (base, object),
