...
from buildworker import version

scripts = ["bin/buildworker"]
...
if 'sdist' in sys.argv or sys.platform == 'win32':
    scripts.append("contrib/windows/buildworker.bat")
...
class our_install_data(install_data):
    ...
    def run(self):
        ...
        # ensure there's a buildworker/VERSION file
        fn = os.path.join(self.install_dir, 'buildworker', 'VERSION')
...
class our_sdist(sdist):
    ...
    def make_release_tree(self, base_dir, files):
        ...
        # ensure there's a buildworker/VERSION file
        fn = os.path.join(base_dir, 'buildworker', 'VERSION')
        open(fn, 'w').write(version)
...
setup_args = {
    ...
    'packages': [
        ...
        "buildworker.scripts",
        "buildworker.monkeypatches",
        "buildworker.test",
        "buildworker.test.fake",
        "buildworker.test.util",
        "buildworker.test.unit",
    ],
    'scripts': scripts,
    # mention data_files, even if empty, so install_data is called and
    # VERSION gets copied
    'data_files': [("buildworker", [])],
    'cmdclass': {
